...
public class ReportFragment extends Fragment {
    ...
    static void dispatch(Activity activity, Lifecycle.Event event) {
        if (activity instanceof LifecycleRegistryOwner) {
            ((LifecycleRegistryOwner) activity).mo959getLifecycle().handleLifecycleEvent(event);
        } else if (!(activity instanceof LifecycleOwner)) {
        } else {
            Lifecycle mo959getLifecycle = ((LifecycleOwner) activity).mo959getLifecycle();
            if (!(mo959getLifecycle instanceof LifecycleRegistry)) {
                return;
            }
            ((LifecycleRegistry) mo959getLifecycle).handleLifecycleEvent(event);
        }
    }
