...
public class AppPermissionsPreferenceController extends BasePreferenceController {
    ...
    public void queryPermissionSummary() {
        for (PackageInfo packageInfo : (List) this.mPackageManager.getInstalledPackages(4096).stream().filter(new AppPermissionsPreferenceController$$ExternalSyntheticLambda0()).limit(4).collect(Collectors.toList())) {
    ...
    public void updateSummary(List<CharSequence> list) {
        ...
        if (i >= 4) {
            ...
            if (list2.isEmpty()) {
                ...
                str = this.mContext.getString(R$string.app_permissions_summary_more, new Object[]{ListFormatter.getInstance().format(list2).toLowerCase()});
            } else {
                str = this.mContext.getString(R$string.app_permissions_summary, new Object[]{ListFormatter.getInstance().format(list2).toLowerCase()});
            }
