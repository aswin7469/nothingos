...
public class TurnScreenOnSettings extends EmptyTextSettings {
    ...
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider(R$xml.turn_screen_on_settings);
        ...
        while (it.hasNext()) {
            ...
            appPreference.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                public boolean onPreferenceClick(Preference preference) {
                    ...
                    AppInfoBase.startAppInfoFragment(TurnScreenOnDetails.class, string, str, i, turnScreenOnSettings, -1, turnScreenOnSettings.getMetricsCategory());
                    return true;
                }
            });
    ...
    public ArrayList<Pair<ApplicationInfo, Integer>> collectTurnScreenOnApps(int i) {
    ...
    static boolean hasTurnScreenOnPermission(PackageManager packageManager, String str) {
        if (!IGNORE_PACKAGE_LIST.contains(str) && packageManager.checkPermission("android.permission.WAKE_LOCK", str) == 0) {
            return true;
        }
        return false;
    }
