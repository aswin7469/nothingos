...
public class ChartDataUsagePreference extends Preference {
    ...
    public int getTop() {
        ...
        return (int) (Math.max(totalUsage, networkPolicy != null ? Math.max(networkPolicy.limitBytes, networkPolicy.warningBytes) : 0L) / 524288);
    }
    ...
    List<DataUsageSummaryNode> getDensedStatsData(List<NetworkCycleData> list) {
        ...
        long max = Math.max(1L, list.stream().mapToLong(ChartDataUsagePreference$$ExternalSyntheticLambda5.INSTANCE).sum());
        long j = 0;
        for (NetworkCycleData networkCycleData : list) {
            j += networkCycleData.getTotalUsage();
        ...
        ((Map) arrayList.stream().collect(Collectors.groupingBy(ChartDataUsagePreference$$ExternalSyntheticLambda1.INSTANCE))).forEach(new BiConsumer() { // from class: com.android.settings.datausage.ChartDataUsagePreference$$ExternalSyntheticLambda0
