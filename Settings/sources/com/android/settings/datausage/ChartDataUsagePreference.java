...
public class ChartDataUsagePreference extends Preference {
    ...
    public int getTop() {
        ...
        return (int) (Math.max(totalUsage, networkPolicy != null ? Math.max(networkPolicy.limitBytes, networkPolicy.warningBytes) : 0) / RESOLUTION);
    }
    ...
    public List<DataUsageSummaryNode> getDensedStatsData(List<NetworkCycleData> list) {
        ...
        long max = Math.max(1, list.stream().mapToLong(new ChartDataUsagePreference$$ExternalSyntheticLambda0()).sum());
        long j = 0;
        for (NetworkCycleData next : list) {
            j += next.getTotalUsage();
        ...
        ((Map) arrayList.stream().collect(Collectors.groupingBy(new ChartDataUsagePreference$$ExternalSyntheticLambda1()))).forEach(new ChartDataUsagePreference$$ExternalSyntheticLambda2(this, arrayList2));
        return (List) arrayList2.stream().sorted(Comparator.comparingInt(new ChartDataUsagePreference$$ExternalSyntheticLambda3())).collect(Collectors.toList());
    }

    /* access modifiers changed from: private */
    public /* synthetic */ void lambda$getDensedStatsData$0(List list, Integer num, List list2) {
        DataUsageSummaryNode dataUsageSummaryNode = new DataUsageSummaryNode(list2.stream().mapToLong(new ChartDataUsagePreference$$ExternalSyntheticLambda4()).min().getAsLong(), list2.stream().mapToLong(new ChartDataUsagePreference$$ExternalSyntheticLambda5()).max().getAsLong(), num.intValue());
        if (list2.size() > 1) {
    ...
    private void bindNetworkPolicy(UsageView usageView, NetworkPolicy networkPolicy, int i) {
        ...
        if (networkPolicy != null) {
            ...
            if (j2 != -1) {
                ...
                float f = ((float) (networkPolicy.warningBytes / j3)) / ((float) i);
