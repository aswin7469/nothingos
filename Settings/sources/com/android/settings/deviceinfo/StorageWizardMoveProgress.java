...
public class StorageWizardMoveProgress extends StorageWizardBase {
    ...
    public CharSequence moveStatusToMessage(int i) {
        if (i == -8) {
            return ((DevicePolicyManager) getSystemService(DevicePolicyManager.class)).getResources().getString("Settings.ERROR_MOVE_DEVICE_ADMIN", new StorageWizardMoveProgress$$ExternalSyntheticLambda0(this));
        }
        if (i == -5) {
        ...
        if (i != -2) {
            return i != -1 ? getString(R$string.insufficient_storage) : getString(R$string.insufficient_storage);
        }
