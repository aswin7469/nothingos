...
public class StorageItemPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, EmptyTrashFragment.OnEmptyTrashCompleteListener {
    ...
    private boolean mIsDocumentsPrefShown = isDocumentsPrefShown();
    ...
    private StorageCacheHelper mStorageCacheHelper = new StorageCacheHelper(this.mContext, this.mUserId);
    ...
    private void updatePrivateStorageCategoryPreferencesOrder() {
        if (this.mScreen != null && isValidPrivateVolume()) {
            ...
            Collections.sort(this.mPrivateStorageItemPreferences, Comparator.comparingLong(new StorageItemPreferenceController$$ExternalSyntheticLambda0()));
            int i = 200;
    ...
    private StorageCacheHelper.StorageCache getSizeInfo(SparseArray<StorageAsyncLoader.StorageResult> sparseArray, int i) {
        ...
        storageCache.systemSize = Math.max(DataUnit.GIBIBYTES.toBytes(1), this.mUsedBytes - j);
        return storageCache;
    }
    ...
    public void onEmptyTrashComplete() {
        ...
        if (storageItemPreference != null) {
            storageItemPreference.setStorageSize(0, this.mTotalSize, true);
            updatePrivateStorageCategoryPreferencesOrder();
        }
    }
}
