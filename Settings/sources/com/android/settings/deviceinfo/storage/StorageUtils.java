...
public class StorageUtils {
    public static List<StorageEntry> getAllStorageEntries(Context context, StorageManager storageManager) {
        ...
        arrayList.addAll((Collection) storageManager.getVolumes().stream().filter(new StorageUtils$$ExternalSyntheticLambda0()).map(new StorageUtils$$ExternalSyntheticLambda1(context)).collect(Collectors.toList()));
        arrayList.addAll((Collection) storageManager.getDisks().stream().filter(new StorageUtils$$ExternalSyntheticLambda2()).map(new StorageUtils$$ExternalSyntheticLambda3()).collect(Collectors.toList()));
        arrayList.addAll((Collection) storageManager.getVolumeRecords().stream().filter(new StorageUtils$$ExternalSyntheticLambda4(storageManager)).map(new StorageUtils$$ExternalSyntheticLambda5()).collect(Collectors.toList()));
        return arrayList;
    }
    ...
    public static String getStorageSizeLabel(Context context, long j) {
        ...
        return TextUtils.expandTemplate(context.getText(R$string.storage_size_large), new CharSequence[]{formatBytes.value, formatBytes.units}).toString();
    }
    ...
    public static class SystemInfoFragment extends InstrumentedDialogFragment {
        ...
        public Dialog onCreateDialog(Bundle bundle) {
            return new AlertDialog.Builder(getActivity()).setMessage((CharSequence) getContext().getString(R$string.storage_detail_dialog_system, new Object[]{Build.VERSION.RELEASE_OR_PREVIEW_DISPLAY})).setPositiveButton(17039370, (DialogInterface.OnClickListener) null).create();
        }
    }

    public static String getStorageSummary(Context context, int i, long j) {
        Formatter.BytesResult formatBytes = Formatter.formatBytes(context.getResources(), j, 1);
        return context.getString(i, new Object[]{formatBytes.value, formatBytes.units});
    }
