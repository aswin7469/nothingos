...
public class TopLevelWallpaperPreferenceController extends BasePreferenceController {
    ...
    public String getTitle() {
        return this.mContext.getString(R$string.nothing_wallpaper_style_customisation);
    }
    ...
    public void updateState(Preference preference) {
        disablePreferenceIfManaged((HomepagePreference) preference);
    }
    ...
    private void disablePreferenceIfManaged(HomepagePreference homepagePreference) {
        if (homepagePreference != null) {
            boolean z = false;
            if (RestrictedLockUtilsInternal.hasBaseUserRestriction(this.mContext, "no_set_wallpaper", UserHandle.myUserId())) {
                homepagePreference.setEnabled(false);
                return;
            }
            if (RestrictedLockUtilsInternal.checkIfRestrictionEnforced(this.mContext, "no_set_wallpaper", UserHandle.myUserId()) != null) {
                z = true;
            }
            homepagePreference.setEnabled(!z);
        }
    }
}
