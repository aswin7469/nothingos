...
public class BatteryTipDialogFragment extends InstrumentedDialogFragment implements DialogInterface.OnClickListener {
    ...
    public Dialog onCreateDialog(Bundle bundle) {
        ...
        if (type == 1) {
            ...
            AlertDialog.Builder negativeButton = new AlertDialog.Builder(context).setTitle((CharSequence) context.getResources().getQuantityString(R$plurals.battery_tip_restrict_app_dialog_title, size, new Object[]{Integer.valueOf(size)})).setPositiveButton(R$string.battery_tip_restrict_app_dialog_ok, (DialogInterface.OnClickListener) this).setNegativeButton(17039360, (DialogInterface.OnClickListener) null);
            if (size == 1) {
            ...
            if (type == 7) {
                ...
                return new AlertDialog.Builder(context).setTitle((CharSequence) getString(R$string.battery_tip_unrestrict_app_dialog_title)).setMessage(R$string.battery_tip_unrestrict_app_dialog_message).setPositiveButton(R$string.battery_tip_unrestrict_app_dialog_ok, (DialogInterface.OnClickListener) this).setNegativeButton(R$string.battery_tip_unrestrict_app_dialog_cancel, (DialogInterface.OnClickListener) null).create();
            } else if (type == 8) {
                this.mMetricsFeatureProvider.action(context, 1771, this.mMetricsKey);
                String format = NumberFormat.getPercentInstance().format(0.800000011920929d);
                String string = context.getString(R$string.battery_tip_limited_temporarily_dialog_msg, new Object[]{format});
                boolean isPluggedIn = isPluggedIn();
                AlertDialog.Builder message = new AlertDialog.Builder(context).setTitle(R$string.battery_tip_limited_temporarily_title).setMessage((CharSequence) string);
                if (isPluggedIn) {
                    message.setPositiveButton(R$string.battery_tip_limited_temporarily_dialog_resume_charge, (DialogInterface.OnClickListener) this).setNegativeButton(R$string.okay, (DialogInterface.OnClickListener) null);
                } else {
