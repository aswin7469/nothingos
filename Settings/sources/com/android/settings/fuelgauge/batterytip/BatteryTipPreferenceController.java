...
public class BatteryTipPreferenceController extends BasePreferenceController {
    ...
    public BatteryTip getCurrentBatteryTip() {
        ...
        if (list != null && list.stream().anyMatch(new BatteryTipPreferenceController$$ExternalSyntheticLambda0())) {
            return (BatteryTip) this.mBatteryTips.stream().filter(new BatteryTipPreferenceController$$ExternalSyntheticLambda0()).findFirst().get();
        }
        return null;
    }
}
