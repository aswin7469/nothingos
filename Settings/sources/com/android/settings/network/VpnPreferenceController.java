...
public class VpnPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, LifecycleObserver, OnResume, OnPause {
    ...
    public int getNumberOfNonLegacyVpn(UserManager userManager, VpnManager vpnManager) {
        return (int) userManager.getUsers().stream().map(new VpnPreferenceController$$ExternalSyntheticLambda3(vpnManager)).filter(new VpnPreferenceController$$ExternalSyntheticLambda4()).count();
    }
    ...
    public String getInsecureVpnSummaryOverride(UserManager userManager, VpnManager vpnManager) {
        ...
        if (insecureVpnCount == 1) {
            return this.mContext.getString(R$string.vpn_settings_single_insecure_multiple_total, new Object[]{Integer.valueOf(insecureVpnCount)});
        }
        return this.mContext.getString(R$string.vpn_settings_multiple_insecure_multiple_total, new Object[]{Integer.valueOf(insecureVpnCount)});
    }
    ...
    public int getInsecureVpnCount(String[] strArr) {
        return (int) Arrays.stream(strArr).map(new VpnPreferenceController$$ExternalSyntheticLambda1()).filter(new VpnPreferenceController$$ExternalSyntheticLambda2()).count();
    }
}
