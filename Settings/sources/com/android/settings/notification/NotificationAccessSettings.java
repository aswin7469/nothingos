...
public class NotificationAccessSettings extends EmptyTextSettings {
    ...
    public void onCreate(Bundle bundle) {
        ...
        build.addCallback(new NotificationAccessSettings$$ExternalSyntheticLambda0(this));
        if (UserManager.get(this.mContext).isManagedProfile()) {
            Toast.makeText(this.mContext, this.mDpm.getResources().getString("Settings.WORK_APPS_CANNOT_ACCESS_NOTIFICATION_SETTINGS", new NotificationAccessSettings$$ExternalSyntheticLambda1(this)), 0).show();
            finish();
        }
    }
    ...
    public void updateList(List<ServiceInfo> list) {
        ...
        for (ServiceInfo next : list) {
            ...
            if (managedProfileId != -10000 && !this.mDpm.isNotificationListenerServicePermitted(next.packageName, managedProfileId)) {
                appPreference.setSummary((CharSequence) this.mDpm.getResources().getString("Settings.WORK_PROFILE_NOTIFICATION_LISTENER_BLOCKED", new NotificationAccessSettings$$ExternalSyntheticLambda2(this)));
            }
            appPreference.setOnPreferenceClickListener(new NotificationAccessSettings$$ExternalSyntheticLambda3(this, componentName, next));
            appPreference.setKey(componentName.flattenToString());
