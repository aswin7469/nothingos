...
public class RemoteVolumeGroupController extends BasePreferenceController implements Preference.OnPreferenceChangeListener, LifecycleObserver, OnDestroy, LocalMediaManager.DeviceCallback {
    ...
    private synchronized void refreshPreference() {
        ...
        for (RoutingSessionInfo next : this.mRoutingSessionInfos) {
            ...
            if (remoteVolumeSeekBarPreference != null) {
                if (remoteVolumeSeekBarPreference.getProgress() != next.getVolume()) {
                    remoteVolumeSeekBarPreference.setProgress(next.getVolume());
                }
                remoteVolumeSeekBarPreference.setEnabled(this.mLocalMediaManager.shouldEnableVolumeSeekBar(next));
            } else {
                ...
                remoteVolumeSeekBarPreference2.setEnabled(this.mLocalMediaManager.shouldEnableVolumeSeekBar(next));
    ...
    public boolean onPreferenceChange(Preference preference, Object obj) {
        ThreadUtils.postOnBackgroundThread((Runnable) new RemoteVolumeGroupController$$ExternalSyntheticLambda0(this, preference, obj));
        return true;
    }
    ...
    public void onDeviceListUpdate(List<MediaDevice> list) {
        if (this.mPreferenceCategory != null) {
            ThreadUtils.postOnMainThread(new RemoteVolumeGroupController$$ExternalSyntheticLambda1(this));
        }
    }
