...
public class ZenRuleScheduleHelper {
    ...
    public String getShortDaysSummary(Context context, ZenModeConfig.ScheduleInfo scheduleInfo) {
        ...
        while (i3 < daysOfWeekForLocale.length) {
            ...
            if (z) {
                if (sb.length() > 0) {
                    sb.append(context2.getString(R$string.summary_divider_text));
                }
                if (i2 == i) {
                    ...
                    sb.append(context2.getString(R$string.summary_range_symbol_combination, new Object[]{this.mDayFormat.format(instance.getTime()), this.mDayFormat.format(instance2.getTime())}));
                    i3++;
                }
            }
            i3++;
    ...
    public String getDaysAndTimeSummary(Context context, ZenModeConfig.ScheduleInfo scheduleInfo) {
        ...
        sb.append(context.getString(R$string.summary_divider_text));
        sb.append(context.getString(R$string.summary_range_symbol_combination, new Object[]{timeString(context, scheduleInfo.startHour, scheduleInfo.startMinute), timeString(context, scheduleInfo.endHour, scheduleInfo.endMinute)}));
        return sb.toString();
    }
}
