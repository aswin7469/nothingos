...
public class AddSupervisedUserActivity extends Activity {
    ...
    public void onCreate(Bundle bundle) {
        ...
        findViewById(R$id.createSupervisedUser).setOnClickListener(new AddSupervisedUserActivity$$ExternalSyntheticLambda0(this));
    }
    ...
    private void createUserAsync(NewUserRequest newUserRequest, Consumer<NewUserResponse> consumer) {
        ...
        Executors.newSingleThreadExecutor().execute(new AddSupervisedUserActivity$$ExternalSyntheticLambda2(this, newUserRequest, new Handler(Looper.getMainLooper()), consumer));
    }

    /* access modifiers changed from: private */
    public /* synthetic */ void lambda$createUserAsync$2(NewUserRequest newUserRequest, Handler handler, Consumer consumer) {
        handler.post(new AddSupervisedUserActivity$$ExternalSyntheticLambda3(consumer, this.mUserManager.createUser(newUserRequest)));
    }

    private void createUser() {
        NewUserRequest build = new NewUserRequest.Builder().setName(getString(R$string.user_new_user_name)).build();
        AlertDialog create = new AlertDialog.Builder(this).setMessage(getString(R$string.creating_new_user_dialog_message)).setCancelable(false).create();
        create.show();
        createUserAsync(build, new AddSupervisedUserActivity$$ExternalSyntheticLambda1(this, create));
    }
