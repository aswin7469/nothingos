...
class LicenseHtmlGeneratorFromXml {
    ...
    static void parse(InputStreamReader inputStreamReader, Map<String, Map<String, Set<String>>> map, Map<String, String> map2) throws XmlPullParserException, IOException {
                ...
                if ("file-name".equals(newPullParser.getName())) {
                    ...
                    if (!TextUtils.isEmpty(attributeValue)) {
                        ...
                        if (!TextUtils.isEmpty(trim)) {
                            ((Set) ((Map) hashMap.computeIfAbsent(trim, new LicenseHtmlGeneratorFromXml$$ExternalSyntheticLambda2())).computeIfAbsent(attributeValue2, new LicenseHtmlGeneratorFromXml$$ExternalSyntheticLambda3())).add(attributeValue);
                        }
                    }
        ...
        for (Map.Entry entry : hashMap.entrySet()) {
            map.merge((String) entry.getKey(), (Map) entry.getValue(), new LicenseHtmlGeneratorFromXml$$ExternalSyntheticLambda4());
        }
    ...
    public static /* synthetic */ Map lambda$parse$3(Map map, Map map2) {
        for (Map.Entry entry : map2.entrySet()) {
            map.merge((String) entry.getKey(), (Set) entry.getValue(), new LicenseHtmlGeneratorFromXml$$ExternalSyntheticLambda5());
        }
        return map;
    }
    ...
    static void generateHtml(Map<String, Map<String, Set<String>>> map, Map<String, String> map2, PrintWriter printWriter, String str) {
        ...
        for (Map<String, Set<String>> entrySet : map.values()) {
            for (Map.Entry entry : entrySet.entrySet()) {
                if (!TextUtils.isEmpty((CharSequence) entry.getKey())) {
                    treeMap.merge((String) entry.getKey(), (Set) entry.getValue(), new LicenseHtmlGeneratorFromXml$$ExternalSyntheticLambda0());
                }
            }
        }
        ...
        printWriter2.println("<ul class=\"files\">");
        for (String str4 : arrayList) {
            for (Map.Entry entry3 : map.get(str4).entrySet()) {
                ...
                for (String str6 : (Set) entry3.getValue()) {
                    ...
                    ((ContentIdAndFileNames) arrayList2.get(intValue)).mLibraryToFileNameMap.computeIfAbsent(str5, new LicenseHtmlGeneratorFromXml$$ExternalSyntheticLambda1()).add(str4);
