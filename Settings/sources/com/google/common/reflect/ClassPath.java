    ...
    static abstract class Scanner {
        ...
        private void scanJar(File file, ClassLoader classLoader) throws IOException {
            try {
                ...
                try {
                    UnmodifiableIterator<File> mo759iterator = getClassPathFromManifest(file, jarFile.getManifest()).mo759iterator();
                    while (mo759iterator.hasNext()) {
                        scan(mo759iterator.next(), classLoader);
                    }
                ...
                for (String str : ClassPath.CLASS_PATH_ATTRIBUTE_SEPARATOR.split(value)) {
                    try {
                        ...
                        if (classPathEntry.getProtocol().equals("file")) {
                            builder.mo798add((ImmutableSet.Builder) ClassPath.toFile(classPathEntry));
                        }
        ...
        static ImmutableMap<File, ClassLoader> getClassPathEntries(ClassLoader classLoader) {
            ...
            UnmodifiableIterator<URL> mo831iterator = getClassLoaderUrls(classLoader).mo831iterator();
            while (mo831iterator.hasNext()) {
                URL next = mo831iterator.next();
        ...
        static ImmutableList<URL> parseJavaClassPath() {
            ...
            for (String str : Splitter.on(StandardSystemProperty.PATH_SEPARATOR.value()).split(StandardSystemProperty.JAVA_CLASS_PATH.value())) {
                try {
                    try {
                        builder.mo798add((ImmutableList.Builder) new File(str).toURI().toURL());
                    } catch (SecurityException unused) {
                        builder.mo798add((ImmutableList.Builder) new URL("file", (String) null, new File(str).getAbsolutePath()));
                    }
