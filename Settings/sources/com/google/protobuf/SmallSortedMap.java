...
public class SmallSortedMap<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public V put(K k, V v) {
        ...
        if (size == i2) {
            ...
            getOverflowEntriesMutable().put((K) remove.mo924getKey(), remove.getValue());
        }
    ...
    private int binarySearchInArray(K k) {
        ...
        if (size >= 0) {
            int compareTo = k.compareTo(this.entryList.get(size).mo924getKey());
            if (compareTo > 0) {
                return -(size + 2);
            }
            if (compareTo == 0) {
        ...
        while (i <= size) {
            int i2 = (i + size) / 2;
            int compareTo2 = k.compareTo(this.entryList.get(i2).mo924getKey());
            if (compareTo2 < 0) {
                size = i2 - 1;
    ...
    public class Entry implements Map.Entry<K, V>, Comparable<SmallSortedMap<K, V>.Entry> {
        ...
        public int compareTo(SmallSortedMap<K, V>.Entry entry) {
            return mo924getKey().compareTo(entry.mo924getKey());
        }
