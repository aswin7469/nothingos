...
public final class DebugMetadataKt {
    ...
    public static final StackTraceElement getStackTraceElement(@NotNull BaseContinuationImpl baseContinuationImpl) {
        ...
        checkDebugMetadataVersion(1, debugMetadataAnnotation.mo24973v());
        int label = getLabel(baseContinuationImpl);
        if (label < 0) {
            i = -1;
        } else {
            i = debugMetadataAnnotation.mo24971l()[label];
        }
        String moduleName = ModuleNameRetriever.INSTANCE.getModuleName(baseContinuationImpl);
        if (moduleName == null) {
            str = debugMetadataAnnotation.mo24969c();
        } else {
            str = moduleName + '/' + debugMetadataAnnotation.mo24969c();
        }
        return new StackTraceElement(str, debugMetadataAnnotation.mo24972m(), debugMetadataAnnotation.mo24970f(), i);
    }
