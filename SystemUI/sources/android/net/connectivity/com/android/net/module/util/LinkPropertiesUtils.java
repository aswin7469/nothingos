...
public final class LinkPropertiesUtils {
    ...
    public static class CompareResult<T> {
        ...
        public CompareResult(Collection<T> collection, Collection<T> collection2) {
            ...
            if (collection2 != null) {
                for (T next : collection2) {
                    if (!this.removed.remove((Object) next)) {
                        this.added.add(next);
                    }
                }
            }
        ...
        public String toString() {
            return "removed=[" + TextUtils.join(NavigationBarInflaterView.BUTTON_SEPARATOR, this.removed) + "] added=[" + TextUtils.join(NavigationBarInflaterView.BUTTON_SEPARATOR, this.added) + NavigationBarInflaterView.SIZE_MOD_END;
        }
    }
    ...
    public static class CompareOrUpdateResult<K, T> {
        ...
        public CompareOrUpdateResult(Collection<T> collection, Collection<T> collection2, Function<T, K> function) {
            ...
            if (collection2 != null) {
                for (T next2 : collection2) {
                    Object remove = hashMap.remove(function.apply(next2));
                    if (remove == null) {
                        this.added.add(next2);
                    } else if (!remove.equals(next2)) {
                        this.updated.add(next2);
                    }
                }
            }
        ...
        public String toString() {
            return "removed=[" + TextUtils.join(NavigationBarInflaterView.BUTTON_SEPARATOR, this.removed) + "] added=[" + TextUtils.join(NavigationBarInflaterView.BUTTON_SEPARATOR, this.added) + "] updated=[" + TextUtils.join(NavigationBarInflaterView.BUTTON_SEPARATOR, this.updated) + NavigationBarInflaterView.SIZE_MOD_END;
        }
    }
    ...
    public static CompareResult<LinkAddress> compareAddresses(LinkProperties linkProperties, LinkProperties linkProperties2) {
        ...
        if (linkProperties2 != null) {
            list = linkProperties2.getLinkAddresses();
        }
        return new CompareResult<>(linkAddresses, list);
    }
    ...
    public static boolean isIdenticalAddresses(LinkProperties linkProperties, LinkProperties linkProperties2) {
        ...
        List<InetAddress> addresses2 = linkProperties2.getAddresses();
