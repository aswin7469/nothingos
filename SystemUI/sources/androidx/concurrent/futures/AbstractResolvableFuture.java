...
public abstract class AbstractResolvableFuture<V> implements ListenableFuture<V> {
        ...
        if (!Thread.interrupted()) {
            ...
            if (nanos + 1000 < 0) {
                ...
                if (i > 0) {
                    ...
                    if (z) {
                        str3 = str3 + NavigationBarInflaterView.BUTTON_SEPARATOR;
                    }
    ...
    public String toString() {
        ...
        if (isCancelled()) {
            ...
            if (str != null && !str.isEmpty()) {
                append.append("PENDING, info=[").append(str).append(NavigationBarInflaterView.SIZE_MOD_END);
            } else if (isDone()) {
                addDoneString(append);
            } else {
        ...
        return append.append(NavigationBarInflaterView.SIZE_MOD_END).toString();
    }
    ...
    public String pendingToString() {
        Object obj = this.value;
        if (obj instanceof SetFuture) {
            return "setFuture=[" + userObjectToString(((SetFuture) obj).future) + NavigationBarInflaterView.SIZE_MOD_END;
        }
    ...
    private void addDoneString(StringBuilder sb) {
        try {
            sb.append("SUCCESS, result=[").append(userObjectToString(getUninterruptibly(this))).append(NavigationBarInflaterView.SIZE_MOD_END);
        } catch (ExecutionException e) {
            sb.append("FAILURE, cause=[").append((Object) e.getCause()).append(NavigationBarInflaterView.SIZE_MOD_END);
        } catch (CancellationException unused) {
            sb.append("CANCELLED");
        } catch (RuntimeException e2) {
            sb.append("UNKNOWN, cause=[").append((Object) e2.getClass()).append(" thrown from get()]");
        }
    }
