...
public class SolverVariableValues implements ArrayRow.ArrayRowVariables {
    ...
    public void display() {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            ...
            if (variable != null) {
                System.out.print(variable + " = " + getVariableValue(i2) + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER);
            }
        }
        System.out.println(" }");
    }
    ...
    public String toString() {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            SolverVariable variable = getVariable(i2);
            if (variable != null) {
                String str4 = str3 + variable + " = " + getVariableValue(i2) + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER;
                ...
                str3 = str2 + NavigationBarInflaterView.SIZE_MOD_END;
            }
        }
        return str3 + " }";
    }
    ...
    private void addToHashMap(SolverVariable solverVariable, int i) {
        int[] iArr;
        int i2 = solverVariable.f73id % this.HASH_SIZE;
        int[] iArr2 = this.keys;
        int i3 = iArr2[i2];
        if (i3 == -1) {
            iArr2[i2] = i;
    ...
    private void displayHash() {
        for (int i = 0; i < this.HASH_SIZE; i++) {
            if (this.keys[i] != -1) {
                ...
                boolean z = false;
                while (!z) {
                    str = str + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + this.variables[i2];
    ...
    private void addVariable(int i, SolverVariable solverVariable, float f) {
        this.variables[i] = solverVariable.f73id;
        this.values[i] = f;
        this.previous[i] = -1;
        this.next[i] = -1;
    ...
    public void put(SolverVariable solverVariable, float f) {
        ...
        if (f <= (-f2) || f >= f2) {
            ...
            for (int i4 = 0; i4 < i; i4++) {
                if (this.variables[i2] == solverVariable.f73id) {
                    this.values[i2] = f;
                    return;
                }
                if (this.variables[i2] < solverVariable.f73id) {
