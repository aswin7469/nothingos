...
public class LinearCurveFit extends CurveFit {
    ...
    /* renamed from: mT */
    private double[] f82mT;
    private double mTotalLength = Double.NaN;

    /* renamed from: mY */
    private double[][] f83mY;
    ...
    private double getLength2D(double d) {
        ...
        int i2 = 0;
        while (i2 < i) {
            double[] dArr2 = this.f83mY[i2];
            double d5 = dArr2[0];
            double d6 = dArr2[1];
            if (i2 > 0) {
                d2 += Math.hypot(d5 - d3, d6 - d4);
            }
            double[] dArr3 = this.f82mT;
            double d7 = dArr3[i2];
            if (d == d7) {
                return d2;
            }
    ...
    public void getPos(double d, double[] dArr) {
        double[] dArr2 = this.f82mT;
        int length = dArr2.length;
        int i = 0;
        int length2 = this.f83mY[0].length;
        if (this.mExtrapolate) {
            double d2 = dArr2[0];
            if (d <= d2) {
                getSlope(d2, this.mSlopeTemp);
                for (int i2 = 0; i2 < length2; i2++) {
                    dArr[i2] = this.f83mY[0][i2] + ((d - this.f82mT[0]) * this.mSlopeTemp[i2]);
                }
                return;
            }
            int i3 = length - 1;
            ...
            if (d >= d3) {
                ...
                while (i < length2) {
                    dArr[i] = this.f83mY[i3][i] + ((d - this.f82mT[i3]) * this.mSlopeTemp[i]);
                    i++;
                }
                return;
            }
    ...
    public void getPos(double d, float[] fArr) {
        ...
        if (this.mExtrapolate) {
            ...
            if (d <= d2) {
                getSlope(d2, this.mSlopeTemp);
                for (int i2 = 0; i2 < length2; i2++) {
                    fArr[i2] = (float) (this.f83mY[0][i2] + ((d - this.f82mT[0]) * this.mSlopeTemp[i2]));
                }
                return;
            }
            int i3 = length - 1;
            ...
            if (d >= d3) {
                getSlope(d3, this.mSlopeTemp);
                while (i < length2) {
                    fArr[i] = (float) (this.f83mY[i3][i] + ((d - this.f82mT[i3]) * this.mSlopeTemp[i]));
                    i++;
                }
                return;
            }
        ...
        while (i6 < length - 1) {
            if (d == this.f82mT[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    fArr[i7] = (float) this.f83mY[i6][i7];
                }
            }
            double[] dArr2 = this.f82mT;
            int i8 = i6 + 1;
    ...
    public double getPos(double d, int i) {
        ...
        if (this.mExtrapolate) {
            double d2 = dArr[0];
            if (d <= d2) {
                return this.f83mY[0][i] + ((d - d2) * getSlope(d2, i));
            }
            int i3 = length - 1;
            double d3 = dArr[i3];
            if (d >= d3) {
                return this.f83mY[i3][i] + ((d - d3) * getSlope(d3, i));
            }
        } else if (d <= dArr[0]) {
            return this.f83mY[0][i];
        } else {
            int i4 = length - 1;
            if (d >= dArr[i4]) {
                return this.f83mY[i4][i];
            }
        }
        while (i2 < length - 1) {
            double[] dArr2 = this.f82mT;
