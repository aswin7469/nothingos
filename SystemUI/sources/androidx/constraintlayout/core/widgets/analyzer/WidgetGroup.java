...
public class WidgetGroup {
    ...
    private String getOrientationString() {
        ...
        return i == 2 ? "Both" : FooterBarMixinMetrics.FooterButtonVisibility.UNKNOWN;
    }

    public String toString() {
        String str = getOrientationString() + " [" + this.f99id + "] <";
        Iterator<ConstraintWidget> it = this.widgets.iterator();
        while (it.hasNext()) {
            str = str + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + it.next().getDebugName();
        }
        return str + " >";
    }
    ...
    public void cleanup(ArrayList<WidgetGroup> arrayList) {
        ...
        if (this.moveTo != -1 && size > 0) {
            for (int i = 0; i < arrayList.size(); i++) {
                ...
                if (this.moveTo == widgetGroup.f99id) {
