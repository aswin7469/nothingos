...
public class CircularFlow extends VirtualLayout {
    ...
    public void init(AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                if (index == C0483R.styleable.ConstraintLayout_Layout_circularflow_viewCenter) {
                    this.mViewCenter = obtainStyledAttributes.getResourceId(index, 0);
                } else if (index == C0483R.styleable.ConstraintLayout_Layout_circularflow_angles) {
                    String string = obtainStyledAttributes.getString(index);
                    this.mReferenceAngles = string;
                    setAngles(string);
                } else if (index == C0483R.styleable.ConstraintLayout_Layout_circularflow_radiusInDP) {
                    String string2 = obtainStyledAttributes.getString(index);
                    this.mReferenceRadius = string2;
                    setRadius(string2);
                } else if (index == C0483R.styleable.ConstraintLayout_Layout_circularflow_defaultAngle) {
                    Float valueOf = Float.valueOf(obtainStyledAttributes.getFloat(index, DEFAULT_ANGLE));
                    this.mReferenceDefaultAngle = valueOf;
                    setDefaultAngle(valueOf.floatValue());
                } else if (index == C0483R.styleable.ConstraintLayout_Layout_circularflow_defaultRadius) {
    ...
    public void addViewToCircularFlow(View view, int i, float f) {
        if (!containsId(view.getId())) {
            ...
            int[] radius = getRadius();
            this.mRadius = radius;
            radius[this.mCountRadius - 1] = (int) (((float) i) * this.myContext.getResources().getDisplayMetrics().density);
            anchorReferences();
        }
    }
    ...
    public void updateRadius(View view, int i) {
        ...
        if (indexFromId <= this.mRadius.length) {
            int[] radius = getRadius();
            this.mRadius = radius;
            radius[indexFromId] = (int) (((float) i) * this.myContext.getResources().getDisplayMetrics().density);
            anchorReferences();
        }
    }
