...
public class ReportFragment extends Fragment {
    ...
    static void dispatch(Activity activity, Lifecycle.Event event) {
        if (activity instanceof LifecycleRegistryOwner) {
            ((LifecycleRegistryOwner) activity).mo1437getLifecycle().handleLifecycleEvent(event);
        } else if (!(activity instanceof LifecycleOwner)) {
        } else {
            Lifecycle mo1437getLifecycle = ((LifecycleOwner) activity).mo1437getLifecycle();
            if (!(mo1437getLifecycle instanceof LifecycleRegistry)) {
                return;
            }
            ((LifecycleRegistry) mo1437getLifecycle).handleLifecycleEvent(event);
        }
    }
