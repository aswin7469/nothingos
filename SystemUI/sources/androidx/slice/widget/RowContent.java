...
public class RowContent extends SliceContent {
    ...
    private boolean populate(SliceItem sliceItem, boolean z) {
        ...
        if ("range".equals(sliceItem.getSubType())) {
            if (SliceQuery.findSubtype(sliceItem, "action", "range") == null || z2) {
                ...
                filterInvalidItems.remove((Object) this.mStartItem);
                if (filterInvalidItems.size() != 1) {
                    ...
                    filterInvalidItems2.remove((Object) getInputRangeThumb());
                    filterInvalidItems.remove((Object) this.mRange);
                    filterInvalidItems.addAll(filterInvalidItems2);
                } else if (isValidRow(filterInvalidItems.get(0))) {
                    sliceItem = filterInvalidItems.get(0);
                    filterInvalidItems = filterInvalidItems(sliceItem);
                    this.mRange = sliceItem;
                    filterInvalidItems.remove((Object) getInputRangeThumb());
                }
            }
        }
        ...
        if (filterInvalidItems.size() > 0) {
            ...
            if (sliceItem2 != null) {
                filterInvalidItems.remove((Object) sliceItem2);
            }
            SliceItem sliceItem3 = this.mPrimaryAction;
            if (sliceItem3 != null) {
                filterInvalidItems.remove((Object) sliceItem3);
            }
    ...
    private static boolean isValidRowContent(SliceItem sliceItem, SliceItem sliceItem2) {
        if (sliceItem2.hasAnyHints(BaseIconCache.IconDB.COLUMN_KEYWORDS, "ttl", "last_updated", "horizontal") || "content_description".equals(sliceItem2.getSubType()) || SliceHints.SUBTYPE_SELECTION_OPTION_KEY.equals(sliceItem2.getSubType()) || SliceHints.SUBTYPE_SELECTION_OPTION_VALUE.equals(sliceItem2.getSubType())) {
            return false;
        }
        String format = sliceItem2.getFormat();
        if ("image".equals(format) || "text".equals(format) || "long".equals(format) || "action".equals(format) || "input".equals(format) || SliceProviderCompat.EXTRA_SLICE.equals(format) || ("int".equals(format) && "range".equals(sliceItem.getSubType()))) {
            return true;
        }
        return false;
    }
