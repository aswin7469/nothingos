...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f172mX;

    /* renamed from: mY */
    private float[] f173mY;
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f172mX = new float[min];
            this.f173mY = new float[min];
            ...
            if (((double) Math.abs(this.f172mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f173mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f172mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f173mY[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f172mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            int i6 = min - 1;
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f172mX[0] + NavigationBarInflaterView.BUTTON_SEPARATOR + this.f173mY[0] + " end:" + this.f172mX[i6] + NavigationBarInflaterView.BUTTON_SEPARATOR + this.f173mY[i6]);
        }
