...
public abstract class BaseLayer implements DrawingContent, BaseKeyframeAnimation.AnimationListener, KeyPathElement {
    ...
    static BaseLayer forModel(Layer layer, LottieDrawable lottieDrawable2, LottieComposition lottieComposition) {
        switch (C15422.$SwitchMap$com$airbnb$lottie$model$layer$Layer$LayerType[layer.getLayerType().ordinal()]) {
            case 1:
    ...
    public void removeAnimation(BaseKeyframeAnimation<?, ?> baseKeyframeAnimation) {
        this.animations.remove((Object) baseKeyframeAnimation);
    }
    ...
    public void draw(Canvas canvas, Matrix matrix2, int i) {
        Paint paint;
        C1488L.beginSection(this.drawTraceName);
        if (!this.visible || this.layerModel.isHidden()) {
            C1488L.endSection(this.drawTraceName);
            return;
        }
        buildParentLayerListIfNeeded();
        C1488L.beginSection("Layer#parentMatrix");
        this.matrix.reset();
        this.matrix.set(matrix2);
        for (int size = this.parentLayers.size() - 1; size >= 0; size--) {
            this.matrix.preConcat(this.parentLayers.get(size).transform.getMatrix());
        }
        C1488L.endSection("Layer#parentMatrix");
        ...
        if (hasMatteOnThisLayer() || hasMasksOnThisLayer()) {
            C1488L.beginSection("Layer#computeBounds");
            ...
            C1488L.endSection("Layer#computeBounds");
            if (this.rect.width() >= 1.0f && this.rect.height() >= 1.0f) {
                C1488L.beginSection("Layer#saveLayer");
                this.contentPaint.setAlpha(255);
                Utils.saveLayerCompat(canvas, this.rect, this.contentPaint);
                C1488L.endSection("Layer#saveLayer");
                clearCanvas(canvas);
                C1488L.beginSection("Layer#drawLayer");
                drawLayer(canvas, this.matrix, intValue);
                C1488L.endSection("Layer#drawLayer");
                if (hasMasksOnThisLayer()) {
                    applyMasks(canvas, this.matrix);
                }
                if (hasMatteOnThisLayer()) {
                    C1488L.beginSection("Layer#drawMatte");
                    C1488L.beginSection("Layer#saveLayer");
                    Utils.saveLayerCompat(canvas, this.rect, this.mattePaint, 19);
                    C1488L.endSection("Layer#saveLayer");
                    clearCanvas(canvas);
                    this.matteLayer.draw(canvas, matrix2, intValue);
                    C1488L.beginSection("Layer#restoreLayer");
                    canvas.restore();
                    C1488L.endSection("Layer#restoreLayer");
                    C1488L.endSection("Layer#drawMatte");
                }
                C1488L.beginSection("Layer#restoreLayer");
                canvas.restore();
                C1488L.endSection("Layer#restoreLayer");
            ...
            recordRenderTime(C1488L.endSection(this.drawTraceName));
            return;
        }
        this.matrix.preConcat(this.transform.getMatrix());
        C1488L.beginSection("Layer#drawLayer");
    ...
    private void intersectBoundsWithMask(RectF rectF, Matrix matrix2) {
        ...
        if (hasMasksOnThisLayer()) {
            ...
            while (i < size) {
                ...
                int i2 = C15422.$SwitchMap$com$airbnb$lottie$model$content$Mask$MaskMode[mask2.getMaskMode().ordinal()];
                if (i2 != 1 && i2 != 2) {
    ...
    static /* synthetic */ class C15422 {
        ...
        static {
            ...
            L_0x0083:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: com.airbnb.lottie.model.layer.BaseLayer.C15422.<clinit>():void");
        }
    }
