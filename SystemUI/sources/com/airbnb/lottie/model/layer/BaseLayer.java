...
public abstract class BaseLayer implements DrawingContent, BaseKeyframeAnimation.AnimationListener, KeyPathElement {
    ...
    private void setupInOutAnimations() {
        ...
        if (!this.layerModel.getInOutKeyframes().isEmpty()) {
            ...
            if (floatKeyframeAnimation.mo189getValue().floatValue() != 1.0f) {
                z = false;
            }
            setVisible(z);
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        ...
        int intValue = (int) ((((i / 255.0f) * (this.transform.getOpacity() == null ? 100 : this.transform.getOpacity().mo189getValue().intValue())) / 100.0f) * 255.0f);
    ...
    private void intersectBoundsWithMask(RectF rectF, Matrix matrix) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            this.path.set(this.mask.getMaskAnimations().get(i).mo189getValue());
    ...
    private void applyAddMask(Canvas canvas, Matrix matrix, Mask mask, BaseKeyframeAnimation<ShapeData, Path> baseKeyframeAnimation, BaseKeyframeAnimation<Integer, Integer> baseKeyframeAnimation2) {
        this.path.set(baseKeyframeAnimation.mo189getValue());
        this.path.transform(matrix);
        this.contentPaint.setAlpha((int) (baseKeyframeAnimation2.mo189getValue().intValue() * 2.55f));
    ...
    private void applyInvertedSubtractMask(Canvas canvas, Matrix matrix, Mask mask, BaseKeyframeAnimation<ShapeData, Path> baseKeyframeAnimation, BaseKeyframeAnimation<Integer, Integer> baseKeyframeAnimation2) {
        ...
        canvas.drawRect(this.rect, this.contentPaint);
        this.dstOutPaint.setAlpha((int) (baseKeyframeAnimation2.mo189getValue().intValue() * 2.55f));
        this.path.set(baseKeyframeAnimation.mo189getValue());
        this.path.transform(matrix);
        canvas.drawPath(this.path, this.dstOutPaint);
        canvas.restore();
