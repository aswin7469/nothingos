...
public abstract class KeyguardAbsKeyInputViewController<T extends KeyguardAbsKeyInputView> extends KeyguardInputViewController<T> {
    ...
    private final KeyguardAbsKeyInputView.KeyDownListener mKeyDownListener = new KeyguardAbsKeyInputView.KeyDownListener() { // from class: com.android.keyguard.KeyguardAbsKeyInputViewController$$ExternalSyntheticLambda0
        ...
        public final boolean onKeyDown(int i, KeyEvent keyEvent) {
            ...
            lambda$new$0 = KeyguardAbsKeyInputViewController.this.lambda$new$0(i, keyEvent);
            return lambda$new$0;
        }
    };
    ...
    void onPasswordChecked(int i, boolean z, int i2, boolean z2) {
        ...
        if (z) {
            ...
            SystemUIEventUtils.collectUnLockResults(getContext(), "unlock_success", 2);
            return;
        }
        if (z2) {
        ...
        SystemUIEventUtils.collectUnLockResults(getContext(), "unlock_fail", 2);
    }
    ...
    public void onUserInput() {
        ...
        if (this.mView instanceof KeyguardPINView) {
            synchronized (this) {
    ...
    protected void verifyPasswordAndUnlockAuto() {
        int i;
        if (this.mDismissing) {
            return;
        }
        ...
        final int currentUser = KeyguardUpdateMonitor.getCurrentUser();
        try {
            i = Integer.parseInt(SystemProperties.get("persist.sys.password.length", String.valueOf(4)));
        } catch (NumberFormatException e) {
            e.printStackTrace();
            i = 0;
        }
        if (enteredCredential.size() != i) {
            return;
        }
        ((KeyguardAbsKeyInputView) this.mView).setPasswordEntryInputEnabled(false);
        AsyncTask<?, ?, ?> asyncTask = this.mPendingLockCheck;
        ...
        this.mLatencyTracker.onActionStart(4);
        this.mKeyguardUpdateMonitor.setCredentialAttempted();
        this.mPendingLockCheck = LockPatternChecker.checkCredential(this.mLockPatternUtils, enteredCredential, currentUser, new LockPatternChecker.OnCheckCallback() { // from class: com.android.keyguard.KeyguardAbsKeyInputViewController.4
            public void onEarlyMatched() {
                KeyguardAbsKeyInputViewController.this.mLatencyTracker.onActionEnd(3);
                KeyguardAbsKeyInputViewController.this.onPasswordChecked(currentUser, true, 0, true);
                enteredCredential.zeroize();
            ...
            public void onChecked(boolean z, int i2) {
                ...
                keyguardAbsKeyInputViewController.mPendingLockCheck = null;
                if (!z) {
                    keyguardAbsKeyInputViewController.onPasswordChecked(currentUser, false, i2, true);
                }
                enteredCredential.zeroize();
            }
