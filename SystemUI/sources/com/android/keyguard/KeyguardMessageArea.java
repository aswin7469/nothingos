...
public class KeyguardMessageArea extends TextView {
    ...
    public void updateLayout() {
        ...
        Log.i("updateLayout", "updateLayout mBouncerVisible=" + this.mBouncerVisible + ", mAltBouncerShowing=" + this.mAltBouncerShowing);
        if (layoutParams == null) {
            return;
        }
        Log.i("updateLayout", "updateLayout old topMargin=" + layoutParams.topMargin);
        if (this.mAltBouncerShowing) {
            int i = getContext().getResources().getDisplayMetrics().heightPixels;
            Log.i("updateLayout", "heightPixels=" + i);
            if (i > 0) {
                layoutParams.topMargin = ((int) (i * 0.857f)) - getContext().getResources().getDimensionPixelSize(R$dimen.status_bar_height);
                setLayoutParams(layoutParams);
            }
        } else {
            layoutParams.topMargin = getContext().getResources().getDimensionPixelSize(R$dimen.keyguard_lock_padding);
        }
        Log.i("updateLayout", "updateLayout new topMargin=" + layoutParams.topMargin);
        setLayoutParams(layoutParams);
    }
}
