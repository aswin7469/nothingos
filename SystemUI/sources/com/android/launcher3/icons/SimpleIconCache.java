...
public class SimpleIconCache extends BaseIconCache {
    ...
    public static SimpleIconCache getIconCache(Context context) {
        synchronized (CACHE_LOCK) {
            ...
            boolean z = context.getResources().getBoolean(C1693R.bool.simple_cache_enable_im_memory);
            String string = context.getString(C1693R.string.cache_db_name);
            HandlerThread handlerThread = new HandlerThread("simple-icon-cache");
            handlerThread.start();
            SimpleIconCache simpleIconCache2 = new SimpleIconCache(context.getApplicationContext(), string, handlerThread.getLooper(), context.getResources().getConfiguration().densityDpi, context.getResources().getDimensionPixelSize(C1693R.dimen.default_icon_bitmap_size), z);
