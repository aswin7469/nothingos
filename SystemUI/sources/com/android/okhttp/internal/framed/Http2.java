    ...
    static final class Reader implements FrameReader {
        ...
        public boolean nextFrame(FrameReader.Handler handler) throws IOException {
            try {
                ...
                int r1 = Http2.readMedium(this.source);
                if (r1 < 0 || r1 > 16384) {
                    throw Http2.ioException("FRAME_SIZE_ERROR: %s", Integer.valueOf(r1));
                }
                byte readByte = (byte) (this.source.readByte() & 255);
                byte readByte2 = (byte) (this.source.readByte() & 255);
                int readInt = this.source.readInt() & Integer.MAX_VALUE;
                if (Http2.logger.isLoggable(Level.FINE)) {
                    Http2.logger.fine(FrameLogger.formatHeader(true, readInt, r1, readByte, readByte2));
                }
                switch (readByte) {
                    case 0:
                        readData(handler, r1, readByte2, readInt);
                        break;
                    case 1:
                        readHeaders(handler, r1, readByte2, readInt);
                        break;
                    case 2:
                        readPriority(handler, r1, readByte2, readInt);
                        break;
                    case 3:
                        readRstStream(handler, r1, readByte2, readInt);
                        break;
                    case 4:
                        readSettings(handler, r1, readByte2, readInt);
                        break;
                    case 5:
                        readPushPromise(handler, r1, readByte2, readInt);
                        break;
                    case 6:
                        readPing(handler, r1, readByte2, readInt);
                        break;
                    case 7:
                        readGoAway(handler, r1, readByte2, readInt);
                        break;
                    case 8:
                        readWindowUpdate(handler, r1, readByte2, readInt);
                        break;
                    default:
                        this.source.skip((long) r1);
                        break;
                }
                return true;
        ...
        private void readHeaders(FrameReader.Handler handler, int i, byte b, int i2) throws IOException {
            ...
            if (i2 != 0) {
                ...
                if ((b & NetworkStackConstants.TCPHDR_URG) != 0) {
        ...
        static String formatFlags(byte b, byte b2) {
            ...
            if (!(b == 2 || b == 3)) {
                ...
                if (!(b == 7 || b == 8)) {
                    ...
                    if (b == 5 && (b2 & 4) != 0) {
                        return str.replace((CharSequence) "HEADERS", (CharSequence) "PUSH_PROMISE");
                    }
                    if (b != 0 || (b2 & NetworkStackConstants.TCPHDR_URG) == 0) {
                        return str;
                    }
                    return str.replace((CharSequence) "PRIORITY", (CharSequence) "COMPRESSED");
                }
            }
            return BINARY[b2];
        }
