...
public class RootTaskDisplayAreaOrganizer extends DisplayAreaOrganizer {
    ...
    public void unregisterListener(RootTaskDisplayAreaListener rootTaskDisplayAreaListener) {
        for (int size = this.mListeners.size() - 1; size >= 0; size--) {
            List valueAt = this.mListeners.valueAt(size);
            if (valueAt != null) {
                valueAt.remove((Object) rootTaskDisplayAreaListener);
            }
        }
    }
    ...
    public void setPosition(SurfaceControl.Transaction transaction, int i, int i2, int i3) {
        SurfaceControl surfaceControl = this.mLeashes.get(i);
        if (surfaceControl != null) {
            transaction.setPosition(surfaceControl, (float) i2, (float) i3);
            return;
        }
    ...
    private void applyConfigChangesToContext(DisplayAreaInfo displayAreaInfo) {
        ...
        if (display == null) {
            ProtoLog.w(ShellProtoLogGroup.WM_SHELL_TASK_ORG, "The display#%d has been removed. Skip following steps", new Object[]{Integer.valueOf(i)});
            return;
        }
