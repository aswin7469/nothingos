package com.android.p019wm.shell.bubbles;

/* renamed from: com.android.wm.shell.bubbles.BubbleExpandedView$$ExternalSyntheticLambda1 */
/* compiled from: D8$$SyntheticClass */
public final /* synthetic */ class BubbleExpandedView$$ExternalSyntheticLambda1 implements Runnable {
    public final /* synthetic */ BubbleExpandedView f$0;
    public final /* synthetic */ boolean f$1;
    public final /* synthetic */ boolean f$2;
    public final /* synthetic */ float f$3;
    public final /* synthetic */ boolean f$4;
    public final /* synthetic */ boolean f$5;

    public /* synthetic */ BubbleExpandedView$$ExternalSyntheticLambda1(BubbleExpandedView bubbleExpandedView, boolean z, boolean z2, float f, boolean z3, boolean z4) {
        this.f$0 = bubbleExpandedView;
        this.f$1 = z;
        this.f$2 = z2;
        this.f$3 = f;
        this.f$4 = z3;
        this.f$5 = z4;
    }

    public final void run() {
        this.f$0.mo48555xadb67084(this.f$1, this.f$2, this.f$3, this.f$4, this.f$5);
    }
}
