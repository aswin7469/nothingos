...
public class LegacySplitScreenTransitions implements Transitions.TransitionHandler {
    ...
    private void startExampleAnimation(SurfaceControl surfaceControl, boolean z) {
        ...
        ofFloat.addUpdateListener(new LegacySplitScreenTransitions$$ExternalSyntheticLambda4(acquire, surfaceControl, f2, f));
    ...
    public /* synthetic */ void mo49750x59db86a4(SurfaceControl.Transaction transaction, SurfaceControl surfaceControl, float f, ValueAnimator valueAnimator) {
        ...
        this.mTransitions.getMainExecutor().execute(new LegacySplitScreenTransitions$$ExternalSyntheticLambda7(this, valueAnimator));
    }
    ...
    private void startExampleResizeAnimation(SurfaceControl surfaceControl, Rect rect, Rect rect2) {
        ...
        final LegacySplitScreenTransitions$$ExternalSyntheticLambda2 legacySplitScreenTransitions$$ExternalSyntheticLambda2 = new LegacySplitScreenTransitions$$ExternalSyntheticLambda2(this, acquire, surfaceControl, rect2, ofFloat);
    ...
    public /* synthetic */ void mo49752xcd682193(SurfaceControl.Transaction transaction, SurfaceControl surfaceControl, Rect rect, ValueAnimator valueAnimator) {
        ...
        transaction.apply();
        this.mTransactionPool.release(transaction);
        this.mTransitions.getMainExecutor().execute(new LegacySplitScreenTransitions$$ExternalSyntheticLambda6(this, valueAnimator));
    }
            ...
            for (int size = transitionInfo.getChanges().size() - 1; size >= 0; size--) {
                ...
                if (mode == 6) {
                    if (change.getParent() != null) {
                        ...
                        transaction3.reparent(leash, transitionInfo.getRootLeash());
                        transaction3.setLayer(leash, transitionInfo.getChanges().size() - size);
                ...
                if (change.getParent() == null) {
                    if ((iBinder2 == this.mPendingEnter && this.mListener.mPrimary.token.equals(change.getContainer())) || this.mListener.mSecondary.token.equals(change.getContainer())) {
                        transaction3.setWindowCrop(leash, change.getStartAbsBounds().width(), change.getStartAbsBounds().height());
                        if (this.mListener.mPrimary.token.equals(change.getContainer())) {
                            transaction3.setLayer(leash, transitionInfo.getChanges().size() + 1);
                        }
                    }
                    ...
                    if (isOpeningType && (mode == 1 || mode == 3)) {
                        ...
                        if (iBinder2 != this.mPendingDismiss || !this.mDismissFromSnap) {
                            ...
                            transaction3.setAlpha(leash, 0.0f);
                        }
                    }
                }
                z = false;
    ...
    public void dismissSplit(LegacySplitScreenTaskListener legacySplitScreenTaskListener, LegacySplitDisplayLayout legacySplitDisplayLayout, boolean z, boolean z2) {
        ...
        this.mTransitions.getMainExecutor().execute(new LegacySplitScreenTransitions$$ExternalSyntheticLambda0(this, z2, windowContainerTransaction));
    }
