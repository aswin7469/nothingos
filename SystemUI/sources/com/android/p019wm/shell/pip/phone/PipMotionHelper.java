...
public class PipMotionHelper implements PipAppOpsListener.Callback, FloatingContentCoordinator.FloatingContent {
    ...
    private ThreadLocal<FrameCallbackScheduler> mSfSchedulerThreadLocal = ThreadLocal.withInitial(new PipMotionHelper$$ExternalSyntheticLambda2(this));
    ...
    private final Consumer<Rect> mUpdateBoundsCallback = new PipMotionHelper$$ExternalSyntheticLambda3(this);
    ...
    public PipMotionHelper(Context context, PipBoundsState pipBoundsState, PipTaskOrganizer pipTaskOrganizer, PhonePipMenuController phonePipMenuController, PipSnapAlgorithm pipSnapAlgorithm, PipTransitionController pipTransitionController, FloatingContentCoordinator floatingContentCoordinator) {
        ...
        this.mResizePipUpdateListener = new PipMotionHelper$$ExternalSyntheticLambda4(this);
    }
    ...
    public void movePip(Rect rect, boolean z) {
        ...
        if (!this.mSpringingToTouch) {
            ...
            this.mPipTaskOrganizer.scheduleUserResizePip(getBounds(), rect, new PipMotionHelper$$ExternalSyntheticLambda0(this));
            return;
        }
    ...
    public void animateDismiss() {
        this.mTemporaryBoundsPhysicsAnimator.spring(FloatProperties.RECT_Y, (float) (this.mPipBoundsState.getMovementBounds().bottom + (getBounds().height() * 2)), 0.0f, this.mSpringConfig).withEndActions(new PipMotionHelper$$ExternalSyntheticLambda1(this));
    ...
    private void rebuildFlingConfigs() {
        this.mFlingConfigX = new PhysicsAnimator.FlingConfig(DEFAULT_FRICTION, (float) this.mPipBoundsState.getMovementBounds().left, (float) this.mPipBoundsState.getMovementBounds().right);
        this.mFlingConfigY = new PhysicsAnimator.FlingConfig(DEFAULT_FRICTION, (float) this.mPipBoundsState.getMovementBounds().top, (float) this.mPipBoundsState.getMovementBounds().bottom);
        Rect stableInsets = this.mPipBoundsState.getDisplayLayout().stableInsets();
        this.mStashConfigX = new PhysicsAnimator.FlingConfig(DEFAULT_FRICTION, (float) ((this.mPipBoundsState.getStashOffset() - this.mPipBoundsState.getBounds().width()) + stableInsets.left), (float) ((this.mPipBoundsState.getDisplayBounds().right - this.mPipBoundsState.getStashOffset()) - stableInsets.right));
    }
    ...
    private void startBoundsAnimator(float f, float f2, Runnable runnable) {
        ...
        if (!this.mTemporaryBoundsPhysicsAnimator.isRunning()) {
            if (runnable != null) {
                this.mTemporaryBoundsPhysicsAnimator.addUpdateListener(this.mResizePipUpdateListener).withEndActions(new PipMotionHelper$$ExternalSyntheticLambda5(this), runnable);
            } else {
                this.mTemporaryBoundsPhysicsAnimator.addUpdateListener(this.mResizePipUpdateListener).withEndActions(new PipMotionHelper$$ExternalSyntheticLambda5(this));
            }
        }
