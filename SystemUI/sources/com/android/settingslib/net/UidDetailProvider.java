    ...
    private UidDetail buildUidDetail(int i) {
        ...
        if (i3 == -5) {
            ...
            if (UserManager.supportsMultipleUsers()) {
                i2 = C1757R.string.data_usage_uninstalled_apps_users;
            } else {
                i2 = C1757R.string.data_usage_uninstalled_apps;
            }
            uidDetail.label = resources.getString(i2);
            uidDetail.icon = packageManager.getDefaultActivityIcon();
            return uidDetail;
        } else if (i3 == 1000) {
            uidDetail.label = resources.getString(C1757R.string.process_kernel_label);
            uidDetail.icon = packageManager.getDefaultActivityIcon();
            return uidDetail;
        } else if (i3 != 1061) {
                ...
                try {
                    ...
                    if (length == 1) {
                        ...
                        while (i5 < length) {
                            ...
                            ApplicationInfo applicationInfo2 = packageManager2.getApplicationInfo(str, j, userId);
                    ...
                    Log.w(TAG, "Error while building UI detail for uid " + i3, e);
                } catch (RemoteException e2) {
                    Log.w(TAG, "Error while building UI detail for uid " + i3, e2);
                }
            ...
            uidDetail.label = resources.getString(C1757R.string.data_usage_ota);
            uidDetail.icon = this.mContext.getDrawable(C1757R.C1759drawable.ic_system_update);
            return uidDetail;
        }
    }
}
