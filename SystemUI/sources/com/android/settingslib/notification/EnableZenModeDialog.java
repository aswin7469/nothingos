...
public class EnableZenModeDialog {
    ...
    public AlertDialog createDialog() {
        ...
        AlertDialog.Builder positiveButton = new AlertDialog.Builder(this.mContext, this.mThemeResId).setTitle(C1757R.string.zen_mode_settings_turn_on_dialog_title).setPositiveButton(C1757R.string.zen_mode_enable_dialog_turn_on, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialogInterface, int i) {
                ...
                if (EnableZenModeDialog.this.isForever(conditionTagAt.condition)) {
                    ...
                    Slog.d(EnableZenModeDialog.TAG, "Invalid manual condition: " + conditionTagAt.condition);
                }
                EnableZenModeDialog.this.mNotificationManager.setZenMode(1, EnableZenModeDialog.this.getRealConditionId(conditionTagAt.condition), EnableZenModeDialog.TAG);
            }
        });
        if (this.mCancelIsNeutral) {
            positiveButton.setNeutralButton(C1757R.string.cancel, (DialogInterface.OnClickListener) null);
        } else {
            positiveButton.setNegativeButton(C1757R.string.cancel, (DialogInterface.OnClickListener) null);
        }
    ...
    public View getContentView() {
        ...
        View inflate = this.mLayoutInflater.inflate(C1757R.layout.zen_mode_turn_on_dialog_container, (ViewGroup) null);
        ScrollView scrollView = (ScrollView) inflate.findViewById(C1757R.C1760id.container);
        this.mZenRadioGroup = (RadioGroup) scrollView.findViewById(C1757R.C1760id.zen_radio_buttons);
        this.mZenRadioGroupContent = (LinearLayout) scrollView.findViewById(C1757R.C1760id.zen_radio_buttons_content);
        this.mZenAlarmWarning = (TextView) scrollView.findViewById(C1757R.C1760id.zen_alarm_warning);
        for (int i = 0; i < this.MAX_MANUAL_DND_OPTIONS; i++) {
            View inflate2 = this.mLayoutInflater.inflate(C1757R.layout.zen_mode_radio_button, this.mZenRadioGroup, false);
            this.mZenRadioGroup.addView(inflate2);
            inflate2.setId(i);
            View inflate3 = this.mLayoutInflater.inflate(C1757R.layout.zen_mode_condition, this.mZenRadioGroupContent, false);
    ...
    public void bind(Condition condition, View view, int i) {
        ...
        if (condition != null) {
            ...
            if (conditionTag2.f252rb != null) {
                z = false;
            }
            if (conditionTag2.f252rb == null) {
                conditionTag2.f252rb = (RadioButton) this.mZenRadioGroup.getChildAt(i);
            }
            conditionTag2.condition = condition;
            final Uri conditionId = getConditionId(conditionTag2.condition);
            if (DEBUG) {
                Log.d(TAG, "bind i=" + this.mZenRadioGroupContent.indexOfChild(view) + " first=" + z + " condition=" + conditionId);
            }
            conditionTag2.f252rb.setEnabled(z2);
            conditionTag2.f252rb.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                public void onCheckedChanged(CompoundButton compoundButton, boolean z) {
                    if (z) {
                        conditionTag2.f252rb.setChecked(true);
    ...
    public void bindConditions(Condition condition) {
        ...
        if (condition == null) {
            ...
            getConditionTagAt(0).f252rb.setChecked(true);
            bindGenericCountdown();
            bindNextAlarm(getTimeUntilNextAlarmCondition());
        } else if (isAlarm(condition)) {
            bindGenericCountdown();
            bindNextAlarm(condition);
            getConditionTagAt(2).f252rb.setChecked(true);
        } else if (isCountdown(condition)) {
            bindNextAlarm(getTimeUntilNextAlarmCondition());
            bind(condition, this.mZenRadioGroupContent.getChildAt(1), 1);
            getConditionTagAt(1).f252rb.setChecked(true);
        } else {
            Slog.d(TAG, "Invalid manual condition: " + condition);
        }
    }
    ...
    private String foreverSummary(Context context) {
        return context.getString(17041802);
    }
    ...
    private void updateUi(final ConditionTag conditionTag, final View view, Condition condition, boolean z, final int i, Uri uri) {
        ...
        conditionTag.lines.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                conditionTag.f252rb.setChecked(true);
            }
        });
    ...
    public void onClickTimeButton(View view, ConditionTag conditionTag, boolean z, int i) {
        ...
        boolean z2 = z;
        this.mMetricsLogger.logOnClickTimeButton(z2);
        ...
        if (i4 == -1) {
            ...
            condition = ZenModeConfig.toTimeCondition(this.mContext, iArr[max], ActivityManager.getCurrentUser());
        }
    ...
    public String computeAlarmWarningText(Condition condition) {
        ...
        if (condition == null || isForever(condition)) {
            i = C1757R.string.zen_alarm_warning_indef;
        } else {
            long tryParseCountdownConditionId = ZenModeConfig.tryParseCountdownConditionId(condition.id);
            i = (tryParseCountdownConditionId <= currentTimeMillis || nextAlarm >= tryParseCountdownConditionId) ? 0 : C1757R.string.zen_alarm_warning;
        }
        if (i == 0) {
            return null;
        }
    ...
    public String getTime(long j, long j2) {
        ...
        return this.mContext.getResources().getString(z ? C1757R.string.alarm_template : C1757R.string.alarm_template_far, new Object[]{DateFormat.format(DateFormat.getBestDateTimePattern(Locale.getDefault(), z ? is24HourFormat ? android.icu.text.DateFormat.HOUR24_MINUTE : "hma" : is24HourFormat ? "EEEHm" : "EEEhma"), j)});
    }
