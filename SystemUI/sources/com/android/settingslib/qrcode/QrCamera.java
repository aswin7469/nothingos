...
public class QrCamera extends Handler {
    ...
    private static List<BarcodeFormat> FORMATS = null;
    ...
    public boolean startPreview() {
        ...
        this.mCamera.setDisplayOrientation(((this.mCameraOrientation - i) + StackStateAnimator.ANIMATION_DURATION_STANDARD) % StackStateAnimator.ANIMATION_DURATION_STANDARD);
    ...
    private Size getBestPreviewSize(Camera.Parameters parameters) {
        ...
        for (Camera.Size next : parameters.getSupportedPreviewSizes()) {
            ...
            if (next.height * next.width > size.getWidth() * size.getHeight() && (Math.abs(d - ratio) / ratio > MAX_RATIO_DIFF || Math.abs(ratio2 - ratio) / ratio <= MAX_RATIO_DIFF)) {
