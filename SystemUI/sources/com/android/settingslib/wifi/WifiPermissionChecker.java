...
public class WifiPermissionChecker {
    ...
    private boolean checkPermission(String str) {
        if (this.mPackageManager == null || TextUtils.isEmpty(this.mLaunchedPackage)) {
            Log.e(TAG, "Failed to check package permission! {PackageManager:" + this.mPackageManager + ", LaunchedPackage:" + this.mLaunchedPackage + "}");
            return false;
        } else if (this.mPackageManager.checkPermission(str, this.mLaunchedPackage) == 0) {
            return true;
        } else {
            Log.w(TAG, "The launched package does not have the required permission! {LaunchedPackage:" + this.mLaunchedPackage + ", Permission:" + str + "}");
            return false;
        }
    }
