...
public class SwipeHelper implements Gefingerpoken {
    ...
    */
    public boolean onInterceptTouchEvent(MotionEvent motionEvent) {
        ...
        if ((view instanceof ExpandableNotificationRow) && (provider = ((ExpandableNotificationRow) view).getProvider()) != null) {
            ...
            if (action != 1) {
                if (action == 2) {
                    if (this.mTouchedView != null && !this.mLongPressSent) {
                        ...
                        float f2 = pos - this.mInitialTouchPos;
                        float f3 = perpendicularPos - this.mPerpendicularInitialTouchPos;
                        if (motionEvent.getClassification() == 1) {
                            f = this.mPagingTouchSlop * this.mSlopMultiplier;
                        } else {
                            f = this.mPagingTouchSlop;
                        }
                        if (Math.abs(f2) > f && Math.abs(f2) > Math.abs(f3)) {
    ...
    public void dismissChild(final View view, float f, final Runnable runnable, long j, boolean z, long j2, boolean z2) {
        ...
        float f3 = f2;
        if (j2 == 0) {
            j3 = i != 0 ? Math.min(400L, (int) ((Math.abs(f3 - getTranslation(view)) * 1000.0f) / Math.abs(f))) : 200L;
        } else {
            j3 = j2;
        }
        if (!this.mDisableHwLayers) {
    ...
    public void snapChild(final View view, float f, float f2) {
        ...
        Animator viewTranslationAnimator = getViewTranslationAnimator(view, f, new ValueAnimator.AnimatorUpdateListener() { // from class: com.android.systemui.SwipeHelper$$ExternalSyntheticLambda0
            ...
            public final void onAnimationUpdate(ValueAnimator valueAnimator) {
                SwipeHelper.this.lambda$snapChild$0(view, canChildBeDismissed, valueAnimator);
            }
        });
        if (viewTranslationAnimator == null) {
            return;
    ...
    /* JADX WARN: Code restructure failed: missing block: B:21:0x0025, code lost:
        if (getTranslation(r5) != 0.0f) goto L12;
     */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    ...
    */
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (!this.mLongPressSent || this.mMenuRowIntercepting) {
            boolean z = false;
            if (!this.mIsSwiping && !this.mMenuRowIntercepting) {
                    ...
                    if (!callback.canChildBeDismissedInDirection(view, z)) {
                        ...
                        if (abs >= size) {
                            ...
                            if (abs > constrainSwipeStartPosition) {
                                pos = ((int) (constrainSwipeStartPosition * Math.signum(pos))) + (f2 * ((float) Math.sin(((pos - f) / size) * 1.5707963267948966d)));
                            }
                        }
                    }
