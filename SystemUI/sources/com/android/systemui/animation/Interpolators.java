...
public class Interpolators {
    ...
    public static float getOvershootInterpolation(float f, float f2, float f3) {
        ...
        float f4 = 1.0f + f2;
        return MathUtils.max(0.0f, ((float) (1.0d - Math.exp((-(MathUtils.log(f4 / f2) / f3)) * f))) * f4);
    }

    public static float getOvershootInterpolation(float f) {
        return MathUtils.max(0.0f, (float) (1.0d - Math.exp(f * (-4.0f))));
    }

    public static float getNotificationScrimAlpha(float f, boolean z, boolean z2) {
        float constrainedMap;
        if (z2) {
            if (z) {
                constrainedMap = MathUtils.constrainedMap(0.0f, 1.0f, 0.4f, 1.0f, f);
            } else {
                constrainedMap = MathUtils.constrainedMap(0.0f, 1.0f, 0.3f, 0.8f, f);
            }
        } else if (z) {
            constrainedMap = MathUtils.constrainedMap(0.0f, 1.0f, 0.3f, 1.0f, f);
        } else {
            constrainedMap = MathUtils.constrainedMap(0.0f, 1.0f, 0.0f, 0.5f, f);
        }
        float f2 = (constrainedMap * 1.2f) - 0.2f;
        if (f2 <= 0.0f) {
            return 0.0f;
        }
        float f3 = 1.0f - f2;
        return (float) (1.0d - ((1.0d - Math.cos((3.14159f * f3) * f3)) * 0.5d));
    }
}
