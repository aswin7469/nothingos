...
class TileAdapterDelegate extends AccessibilityDelegateCompat {
    ...
    public void onInitializeAccessibilityNodeInfo(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
        ...
        if (holder != null && holder.canTakeAccessibleAction()) {
            ...
            if (holder.isCurrentTile()) {
                accessibilityNodeInfoCompat.setStateDescription(view.getContext().getString(C1894R.string.accessibility_qs_edit_position, new Object[]{Integer.valueOf(holder.getLayoutPosition())}));
            }
        }
    }
    ...
    public boolean performAccessibilityAction(View view, int i, Bundle bundle) {
        ...
        if (i == 16) {
            holder.toggleState();
            return true;
        } else if (i == C1894R.C1898id.accessibility_action_qs_move_to_position) {
            holder.startAccessibleMove();
            return true;
        } else if (i != C1894R.C1898id.accessibility_action_qs_add_to_position) {
            return super.performAccessibilityAction(view, i, bundle);
        } else {
    ...
    private void addClickAction(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat, TileAdapter.Holder holder) {
        String str;
        if (holder.canAdd()) {
            str = view.getContext().getString(C1894R.string.accessibility_qs_edit_tile_add_action);
        } else if (holder.canRemove()) {
            str = view.getContext().getString(C1894R.string.accessibility_qs_edit_remove_tile_action);
        } else {
    ...
    private void maybeAddActionMoveToPosition(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat, TileAdapter.Holder holder) {
        if (holder.isCurrentTile()) {
            accessibilityNodeInfoCompat.addAction(new AccessibilityNodeInfoCompat.AccessibilityActionCompat(C1894R.C1898id.accessibility_action_qs_move_to_position, view.getContext().getString(C1894R.string.accessibility_qs_edit_tile_start_move)));
        }
    }
    ...
    private void maybeAddActionAddToPosition(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat, TileAdapter.Holder holder) {
        if (holder.canAdd()) {
            accessibilityNodeInfoCompat.addAction(new AccessibilityNodeInfoCompat.AccessibilityActionCompat(C1894R.C1898id.accessibility_action_qs_add_to_position, view.getContext().getString(C1894R.string.accessibility_qs_edit_tile_start_add)));
        }
    }
}
