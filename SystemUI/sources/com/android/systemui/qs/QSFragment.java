...
public class QSFragment extends LifecycleFragment implements QS, CommandQueue.Callbacks, StatusBarStateController.StateListener {
    ...
    public QSFragment(RemoteInputQuickSettingsDisabler remoteInputQuickSettingsDisabler, InjectionInflationController injectionInflationController, QSTileHost qSTileHost, StatusBarStateController statusBarStateController, CommandQueue commandQueue, QSDetailDisplayer qSDetailDisplayer, MediaHost mediaHost, MediaHost mediaHost2, KeyguardBypassController keyguardBypassController, QSFragmentComponent.Factory factory, FeatureFlags featureFlags, FalsingManager falsingManager, DumpManager dumpManager) {
        ...
        commandQueue.observe(mo1437getLifecycle(), (Lifecycle) this);
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        LayoutInflater injectable;
        Resources resources = getContext().getResources();
        DisplayMetrics displayMetrics = resources.getDisplayMetrics();
        if (displayMetrics.densityDpi > 420 || displayMetrics.scaledDensity > 3.375f) {
            Configuration configuration = resources.getConfiguration();
            Configuration configuration2 = new Configuration();
            configuration2.updateFrom(configuration);
            if (displayMetrics.densityDpi > 420) {
                configuration2.smallestScreenWidthDp = (int) (Math.min(displayMetrics.widthPixels, displayMetrics.heightPixels) / 2.688f);
                configuration2.screenWidthDp = (int) (displayMetrics.widthPixels / 2.688f);
                configuration2.screenHeightDp = (int) (displayMetrics.heightPixels / 2.688f);
                configuration2.densityDpi = 430;
            ...
            Context createConfigurationContext = getContext().createConfigurationContext(configuration2);
            createConfigurationContext.setTheme(R$style.Theme_SystemUI_QuickSettings);
            injectable = this.mInjectionInflater.injectable(layoutInflater.cloneInContext(createConfigurationContext));
        } else {
            injectable = this.mInjectionInflater.injectable(layoutInflater.cloneInContext(new ContextThemeWrapper(getContext(), R$style.Theme_SystemUI_QuickSettings)));
        }
        return injectable.inflate(R$layout.qs_panel, viewGroup, false);
    }
    ...
    public void onViewCreated(View view, Bundle bundle) {
        ...
        QSAnimator qSAnimator = create.getQSAnimator();
        this.mQSAnimator = qSAnimator;
        if (qSAnimator != null) {
            this.mQSPanelController.setQSAnimator(qSAnimator);
        }
    ...
    public void setQsExpansion(float f, float f2) {
        ...
        if (!this.mHeaderAnimating && !headerWillBeAnimating()) {
            getView().setTranslationY(z3 ? this.mHeader.getHeight() * f5 : f3);
        }
        ...
        this.mHeader.setExpansion(z3, f, f5 * ((this.mQSPanelScrollView.getBottom() - this.mHeader.getBottom()) + this.mHeader.getPaddingBottom()));
        ...
        this.mQSPanelController.setExpansion(f, f2);
        if (z2) {
    ...
    private void updateMediaPositions() {
        if (Utils.useQsMediaPlayer(getContext())) {
            ...
            pinToBottom((height - this.mQSPanelScrollView.getScrollY()) + this.mQSPanelScrollView.getScrollRange(), this.mQsMediaHost, true);
    ...
    private void pinToBottom(float f, MediaHost mediaHost, boolean z) {
        ...
        if (this.mLastQSExpansion > 0.0f && !isKeyguardState() && this.mQqsMediaHost.getVisible()) {
            ...
            if (z) {
                max = Math.min(totalBottomMargin, 0.0f);
            } else {
                max = Math.max(totalBottomMargin, 0.0f);
            }
            hostView.setTranslationY(max);
            return;
        }
    ...
    private float getTotalBottomMargin(View view) {
        ...
        while (true) {
            ...
            i += view.getHeight() - view3.getBottom();
    ...
    private boolean headerWillBeAnimating() {
        return this.mState == 1 && this.mShowCollapsedOnKeyguard && !isKeyguardState();
    }
    ...
    public int getQsMinExpansionHeight() {
        if (SystemUIUtils.getInstance().shouldUseSplitNotificationShade()) {
            return this.mHeader.getHeaderQsPanel().getHeight();
        }
