    ...
    */
            ...
            if (customTransformation == null || !customTransformation.initTransformation(this, transformState)) {
                if (z) {
                    if (transformRightEdge) {
                        setTransformationStartX((locationOnScreen[0] + transformState.getTransformedView().getWidth()) - (laidOutLocationOnScreen[0] + view.getWidth()));
                    } else {
    ...
    private void transformViewTo(TransformState transformState, int i, ViewTransformationHelper.CustomTransformation customTransformation, float f) {
        ...
        if (transformScale) {
            float transformationStartScaleX = getTransformationStartScaleX();
            if (transformationStartScaleX != -1.0f) {
                view.setScaleX(NotificationUtils.interpolate(transformationStartScaleX, contentWidth2 / contentWidth, interpolation));
            }
            float transformationStartScaleY = getTransformationStartScaleY();
            if (transformationStartScaleY == -1.0f) {
                return;
            }
            view.setScaleY(NotificationUtils.interpolate(transformationStartScaleY, transformState.getContentHeight() / getContentHeight(), interpolation));
        }
    }
