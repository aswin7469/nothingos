...
public class BiometricUnlockController extends KeyguardUpdateMonitorCallback implements Dumpable {
    ...
    public BiometricUnlockController(Context context, DozeScrimController dozeScrimController, KeyguardViewMediator keyguardViewMediator, ScrimController scrimController, ShadeController shadeController, NotificationShadeWindowController notificationShadeWindowController, KeyguardStateController keyguardStateController, Handler handler, KeyguardUpdateMonitor keyguardUpdateMonitor, Resources resources, KeyguardBypassController keyguardBypassController, DozeParameters dozeParameters, MetricsLogger metricsLogger, DumpManager dumpManager, PowerManager powerManager, NotificationMediaManager notificationMediaManager, WakefulnessLifecycle wakefulnessLifecycle, ScreenLifecycle screenLifecycle, AuthController authController) {
        ...
        this.mWakeUpDelay = resources.getInteger(17694949);
    ...
    public void startWakeAndUnlock(int i) {
        ...
        final boolean isDeviceInteractive = this.mUpdateMonitor.isDeviceInteractive();
        this.mMode = i;
        if (i == 1) {
        ...
        Runnable runnable = new Runnable() { // from class: com.android.systemui.statusbar.phone.BiometricUnlockController$$ExternalSyntheticLambda1
            ...
            public final void run() {
                BiometricUnlockController.this.lambda$startWakeAndUnlock$0(isDeviceInteractive, z);
            }
        };
        if (!z && this.mMode != 0) {
            runnable.run();
    ...
    public void onFinishedGoingToSleep(int i) {
        ...
        if (pendingAuthenticated != null) {
            this.mHandler.post(new Runnable() { // from class: com.android.systemui.statusbar.phone.BiometricUnlockController$$ExternalSyntheticLambda0
                @Override // java.lang.Runnable
                public final void run() {
                    BiometricUnlockController.this.lambda$onFinishedGoingToSleep$1(pendingAuthenticated);
                }
            });
            this.mPendingAuthenticated = null;
        }
    ...
    private int calculateModeForPassiveAuth(boolean z) {
        ...
        if (!this.mUpdateMonitor.isDeviceInteractive()) {
            return !this.mKeyguardViewController.isShowing() ? z2 ? 1 : 4 : !isUnlockingWithBiometricAllowed ? z2 ? 3 : 0 : this.mDozeScrimController.isPulsing() ? z2 ? 2 : 4 : z2 ? 2 : 4;
        } else if (isUnlockingWithBiometricAllowed && isDreaming) {
            return z2 ? 6 : 4;
        } else if (isUnlockingWithBiometricAllowed && this.mKeyguardStateController.isOccluded()) {
            return 5;
        } else {
            if (!this.mKeyguardViewController.isShowing()) {
                return 0;
            }
            return ((this.mKeyguardViewController.bouncerIsOrWillBeShowing() || this.mKeyguardBypassController.getAltBouncerShowing()) && isUnlockingWithBiometricAllowed) ? (!z2 || !this.mKeyguardBypassController.canPlaySubtleWindowAnimations()) ? 8 : 7 : isUnlockingWithBiometricAllowed ? (z2 || this.mAuthController.isUdfpsFingerDown()) ? 7 : 0 : z2 ? 3 : 0;
        }
    }
