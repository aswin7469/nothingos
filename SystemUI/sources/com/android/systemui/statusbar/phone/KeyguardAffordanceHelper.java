    ...
    */
    private void flingWithCurrentVelocity(boolean z, float f, float f2) {
        ...
        if (this.mCallback.needsAntiFalsing()) {
            if (this.mFalsingManager.isFalseTouch(this.mTargetedView == this.mRightIcon ? 6 : 5)) {
                z2 = true;
                boolean z6 = !z2 || isBelowFalsingThreshold();
                z3 = this.mTranslation * currentVelocity >= 0.0f;
                z4 = z6 | (Math.abs(currentVelocity) <= ((float) this.mMinFlingVelocity) && z3);
                if (z3 ^ z4) {
                ...
                fling(currentVelocity, z7, z5);
            }
        }
        z2 = false;
        if (!z2) {
        }
        if (this.mTranslation * currentVelocity >= 0.0f) {
        }
        z4 = z6 | (Math.abs(currentVelocity) <= ((float) this.mMinFlingVelocity) && z3);
        if (z3 ^ z4) {
        }
    ...
    private float getRadiusFromTranslation(float f) {
        ...
        return ((f - i) * 0.25f) + this.mMinBackgroundRadius;
    }
    ...
    public void launchAffordance(boolean z, boolean z2) {
        ...
        this.mTranslation = maxTranslationDistance;
