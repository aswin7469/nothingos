...
public class WalletCardCarousel extends RecyclerView {
    ...
    public void setExpectedViewWidth(int i) {
        if (this.mExpectedViewWidth != i) {
            ...
            int round = Math.round(((float) Math.min(i, Math.min(displayMetrics.widthPixels, displayMetrics.heightPixels))) * CARD_VIEW_WIDTH_RATIO);
            this.mCardWidthPx = round;
            this.mCardHeightPx = Math.round(((float) round) / CARD_ASPECT_RATIO);
    ...
    public void updateCardView(View view) {
        ...
        float f = right - width;
        float max = Math.max((float) UNSELECTED_CARD_SCALE, 1.0f - Math.abs(f / ((float) view.getWidth())));
    ...
    private class CardCarouselScrollListener extends RecyclerView.OnScrollListener {
        ...
        public void onScrolled(RecyclerView recyclerView, int i, int i2) {
            ...
            float unused = WalletCardCarousel.this.mCardCenterToScreenCenterDistancePx = Float.MAX_VALUE;
            ...
            if (WalletCardCarousel.this.mCenteredAdapterPosition != -1 && i != 0) {
                int i5 = WalletCardCarousel.this.mCenteredAdapterPosition;
                if (WalletCardCarousel.this.mEdgeToCenterDistance > 0.0f) {
                    i3 = 1;
                }
                int i6 = i5 + i3;
                if (i6 >= 0 && i6 < WalletCardCarousel.this.mWalletCardCarouselAdapter.mData.size()) {
