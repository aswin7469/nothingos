...
public class SavedNetworkTracker extends BaseWifiTracker {
    ...
    private void updateStandardWifiEntryScans(List<ScanResult> list) {
        ...
        this.mStandardWifiEntryCache.forEach(new SavedNetworkTracker$$ExternalSyntheticLambda2((Map) list.stream().collect(Collectors.groupingBy(new SavedNetworkTracker$$ExternalSyntheticLambda1()))));
    }
    ...
    private void updatePasspointWifiEntryScans(List<ScanResult> list) {
        ...
        for (Pair next : this.mWifiManager.getAllMatchingWifiConfigs(list)) {
            ...
            if (this.mPasspointWifiEntryCache.containsKey(uniqueIdToPasspointWifiEntryKey)) {
                this.mPasspointWifiEntryCache.get(uniqueIdToPasspointWifiEntryKey).updateScanResultInfo(wifiConfiguration, (List) ((Map) next.second).get(0), (List) ((Map) next.second).get(1));
            }
        }
        for (PasspointWifiEntry next2 : this.mPasspointWifiEntryCache.values()) {
            if (!treeSet.contains(next2.getKey())) {
                next2.updateScanResultInfo((WifiConfiguration) null, (List<ScanResult>) null, (List<ScanResult>) null);
            }
        }
    }
    ...
    private void updateStandardWifiEntryConfigs(List<WifiConfiguration> list) {
        ...
        Map map = (Map) list.stream().filter(new SavedNetworkTracker$$ExternalSyntheticLambda6()).collect(Collectors.groupingBy(new SavedNetworkTracker$$ExternalSyntheticLambda7()));
        this.mStandardWifiEntryCache.removeIf(new SavedNetworkTracker$$ExternalSyntheticLambda8(map));
    ...
    static /* synthetic */ boolean lambda$updateStandardWifiEntryConfigs$1(WifiConfiguration wifiConfiguration) {
        return !wifiConfiguration.carrierMerged;
    }

    private void updatePasspointWifiEntryConfigs(List<PasspointConfiguration> list) {
        Preconditions.checkNotNull(list, "Config list should not be null!");
        Map map = (Map) list.stream().collect(Collectors.toMap(new SavedNetworkTracker$$ExternalSyntheticLambda4(), Function.identity()));
        this.mPasspointWifiEntryCache.entrySet().removeIf(new SavedNetworkTracker$$ExternalSyntheticLambda5(map));
    ...
    static /* synthetic */ boolean lambda$updatePasspointWifiEntryConfigs$4(Map map, Map.Entry entry) {
    ...
    private void notifyOnSubscriptionWifiEntriesChanged() {
        if (this.mListener != null) {
            ...
            SavedNetworkTrackerCallback savedNetworkTrackerCallback = this.mListener;
            Objects.requireNonNull(savedNetworkTrackerCallback);
            handler.post(new SavedNetworkTracker$$ExternalSyntheticLambda3(savedNetworkTrackerCallback));
        }
    }
}
