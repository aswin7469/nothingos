...
public class LegacySplitScreenTaskListener implements ShellTaskOrganizer.TaskListener {
        ...
        synchronized (this) {
            ...
            if (windowingMode == 3) {
                if (ShellProtoLogCache.WM_SHELL_TASK_ORG_enabled) {
                    ShellProtoLogImpl.v(ShellProtoLogGroup.WM_SHELL_TASK_ORG, -1362429294, 4, null, String.valueOf(TAG), Long.valueOf(runningTaskInfo.taskId));
                }
                this.mPrimary = runningTaskInfo;
                this.mPrimarySurface = surfaceControl;
            } else if (windowingMode == 4) {
                if (ShellProtoLogCache.WM_SHELL_TASK_ORG_enabled) {
                    ShellProtoLogImpl.v(ShellProtoLogGroup.WM_SHELL_TASK_ORG, 982027396, 4, null, String.valueOf(TAG), Long.valueOf(runningTaskInfo.taskId));
                }
                this.mSecondary = runningTaskInfo;
                this.mSecondarySurface = surfaceControl;
            } else if (ShellProtoLogCache.WM_SHELL_TASK_ORG_enabled) {
                ShellProtoLogImpl.v(ShellProtoLogGroup.WM_SHELL_TASK_ORG, -298656957, 20, null, String.valueOf(TAG), Long.valueOf(runningTaskInfo.taskId), Long.valueOf(windowingMode));
            }
    ...
    public void onTaskInfoChanged(ActivityManager.RunningTaskInfo runningTaskInfo) {
        ...
        synchronized (this) {
            ...
            if (runningTaskInfo.hasParentTask()) {
                if (runningTaskInfo.positionInParent.equals(this.mPositionByTaskId.get(runningTaskInfo.taskId))) {
                    return;
                }
