...
public class OneHandedDisplayAreaOrganizer extends DisplayAreaOrganizer {
    ...
    private void animateWindows(WindowContainerToken windowContainerToken, SurfaceControl surfaceControl, float f, float f2, int i, int i2) {
        ...
        if (animator != null) {
            animator.setTransitionDirection(i).addOneHandedAnimationCallback(this.mOneHandedAnimationCallback).addOneHandedAnimationCallback(this.mTutorialHandler).addOneHandedAnimationCallback(this.mBackgroundPanelOrganizer).setDuration(i2).start();
        }
    }
