...
public class SplashScreenExitAnimation implements Animator.AnimatorListener {
    ...
    private ValueAnimator createAnimator() {
        ...
        int sqrt = (int) ((((int) Math.sqrt((height * height) + (width * width))) * 1.25f) + 0.5d);
    ...
    public static class RadialVanishAnimation extends View {
        ...
        void onAnimationProgress(float f) {
            ...
            float f2 = i + ((this.mFinishRadius - i) * interpolation);
    ...
    public final class ShiftUpAnimation {
        ...
        void finish() {
            ...
            sfInstance.postCallback(4, new Runnable() { // from class: com.android.wm.shell.startingsurface.SplashScreenExitAnimation$ShiftUpAnimation$$ExternalSyntheticLambda0
    ...
    private void onAnimationProgress(float f) {
        ...
        if (iconView != null) {
            iconView.setAlpha(this.mIconStartAlpha * (1.0f - ICON_INTERPOLATOR.getInterpolation(getProgress(f, 0L, this.mIconFadeOutDuration))));
        }
        float progress = getProgress(f, this.mAppRevealDelay, this.mAppRevealDuration);
    ...
    private float getProgress(float f, long j, long j2) {
        return MathUtils.constrain(((f * this.mAnimationDuration) - ((float) j)) / ((float) j2), 0.0f, 1.0f);
    }
}
