...
final class Cam16 {
    ...
    static Cam16 fromIntInViewingConditions(int i, ViewingConditions viewingConditions) {
        ...
        float pow5 = ((float) Math.pow(1.64d - Math.pow(0.29d, (double) viewingConditions.getN()), 0.73d)) * ((float) Math.pow((double) ((((((((float) (Math.cos(Math.toRadians((double) (((double) atan2) < 20.14d ? 360.0f + atan2 : atan2)) + 2.0d) + 3.8d)) * 0.25f) * 3846.1538f) * viewingConditions.getNc()) * viewingConditions.getNcb()) * ((float) Math.hypot(d3, d2))) / (f11 + 0.305f)), 0.9d));
        ...
        return new Cam16(atan2, sqrt, pow4, c, flRoot, sqrt2, f14, log1p * ((float) Math.cos(d4)), log1p * ((float) Math.sin(d4)));
    }
    ...
    public int viewed(ViewingConditions viewingConditions) {
        ...
        float[] fArr3 = fArr[1];
        float[] fArr4 = fArr[2];
        return ColorUtils.intFromXyzComponents(f11, (fArr3[0] * f8) + (fArr3[1] * f9) + (fArr3[2] * f10), (f8 * fArr4[0]) + (f9 * fArr4[1]) + (f10 * fArr4[2]));
    }
}
