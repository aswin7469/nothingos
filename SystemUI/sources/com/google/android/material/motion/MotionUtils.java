...
public class MotionUtils {
    ...
    public static TimeInterpolator resolveThemeInterpolator(Context context, int i, TimeInterpolator timeInterpolator) {
        ...
        if (typedValue.type == 3) {
            String valueOf = String.valueOf((Object) typedValue.string);
            if (isEasingType(valueOf, EASING_TYPE_CUBIC_BEZIER)) {
                String[] split = getEasingContent(valueOf, EASING_TYPE_CUBIC_BEZIER).split(NavigationBarInflaterView.BUTTON_SEPARATOR);
                if (split.length == 4) {
