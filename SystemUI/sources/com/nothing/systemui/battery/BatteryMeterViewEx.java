...
public class BatteryMeterViewEx {
    ...
    public static void addBatteryImageView(Context context, BatteryMeterView batteryMeterView, ImageView imageView) {
        ...
        FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(context.getResources().getDimensionPixelSize(C1894R.dimen.status_bar_battery_icon_width), context.getResources().getDimensionPixelSize(C1894R.dimen.status_bar_battery_icon_height));
        layoutParams.gravity = 17;
        frameLayout.addView(imageView, layoutParams);
        batteryMeterView.addView(frameLayout, new ViewGroup.MarginLayoutParams(context.getResources().getDimensionPixelSize(C1894R.dimen.battery_icon_container_size), -1));
    }

    public static void refreshByBatteryStateEx(Context context, ImageView imageView, Drawable drawable, boolean z) {
        if (z) {
            NTLogUtil.m1688i("BatteryMeterView", "refreshByBatteryStateEx: batteryStateUnknown");
            return;
        }
        if (sBatteryStateEx.status == 4) {
            if (sBatteryStateEx.voltage > VOLT_MAX) {
                imageView.setImageDrawable(context.getDrawable(C1894R.C1896drawable.ic_battery_alert));
                NTLogUtil.m1688i("BatteryMeterView", "refreshByBatteryStateEx: volt > max");
                return;
            } else if (sBatteryStateEx.temperature >= TEMP_MAX) {
                imageView.setImageDrawable(context.getDrawable(C1894R.C1896drawable.ic_battery_pause));
                NTLogUtil.m1688i("BatteryMeterView", "refreshByBatteryStateEx: temp >= max");
                return;
            } else if (sBatteryStateEx.temperature <= 0) {
                imageView.setImageDrawable(context.getDrawable(C1894R.C1896drawable.ic_battery_pause));
                NTLogUtil.m1688i("BatteryMeterView", "refreshByBatteryStateEx: temp <= min");
                return;
            }
        }
    ...
    public static void updateBatteryStateEx(BatteryStateEx batteryStateEx) {
        ...
        if (z2 || z3 || z) {
            NTLogUtil.m1688i(TAG, "batteryStateExt changed from: " + batteryStateEx + " to " + sBatteryStateEx);
        }
