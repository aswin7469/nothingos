...
public final class GenericSignatureParser {
    ...
    public TypeVariableImpl<GenericDeclaration> parseFormalTypeParameter() {
        ...
        expect(AccessibilityUtils.ENABLED_ACCESSIBILITY_SERVICES_SEPARATOR);
        char c = this.symbol;
        if (c == 'L' || c == '[' || c == 'T') {
    ...
    public Type parseClassTypeSignature() {
        ...
        sb.append(this.identifier);
        ParameterizedTypeImpl parameterizedTypeImpl = new ParameterizedTypeImpl((ParameterizedTypeImpl) null, sb.toString(), parseOptTypeArguments(), this.loader);
        while (this.symbol == '.') {
            scanSymbol();
            scanIdentifier();
            sb.append("$");
            sb.append(this.identifier);
            parameterizedTypeImpl = new ParameterizedTypeImpl(parameterizedTypeImpl, sb.toString(), parseOptTypeArguments(), this.loader);
        }
        expect(';');
    ...
    public void parseMethodTypeSignature(Class<?>[] clsArr) {
        ...
        if (this.symbol == '^') {
            ...
            this.exceptionTypes = new ListOfTypes((Type[]) clsArr);
        } else {
