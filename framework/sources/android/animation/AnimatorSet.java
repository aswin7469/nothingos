...
public final class AnimatorSet extends Animator implements AnimationHandler.AnimationFrameCallback {
    ...
    public AnimatorSet() {
        ...
        boolean z;
        boolean z2 = false;
        ValueAnimator mo1103setDuration = ValueAnimator.ofFloat(0.0f, 1.0f).mo1103setDuration(0L);
        this.mDelayAnim = mo1103setDuration;
        this.mRootNode = new Node(mo1103setDuration);
        ...
        this.mEndCanBeCalled = !isPreO ? true : z2;
    }
    ...
    public AnimatorSet mo1103setDuration(long duration) {
        if (duration < 0) {
    ...
    private void updateAnimatorsDuration() {
        if (this.mDuration >= 0) {
            ...
            for (int i = 0; i < size; i++) {
                Node node = this.mNodes.get(i);
                node.mAnimation.mo1103setDuration(this.mDuration);
            }
        }
        this.mDelayAnim.mo1103setDuration(this.mStartDelay);
    }
    ...
    public void animateBasedOnPlayTime(long currentPlayTime, long lastPlayTime, boolean inReverse) {
        long lastPlayTime2;
        boolean inReverse2;
        long currentPlayTime2 = currentPlayTime;
        if (currentPlayTime2 < 0 || lastPlayTime < 0) {
            throw new UnsupportedOperationException("Error: Play time should never be negative.");
        }
        if (inReverse) {
            if (getTotalDuration() != -1) {
                long duration = getTotalDuration() - this.mStartDelay;
                currentPlayTime2 = duration - Math.min(currentPlayTime2, duration);
    ...
    public AnimatorSet mo1102clone() {
        final AnimatorSet anim = (AnimatorSet) super.m26clone();
        int nodeCount = this.mNodes.size();
        anim.mStarted = false;
    ...
    public static class Node implements Cloneable {
        ...
        /* renamed from: clone */
        public Node m30clone() {
            try {
                Node node = (Node) super.clone();
                node.mAnimation = this.mAnimation.m26clone();
