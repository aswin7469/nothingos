...
import android.content.res.iconhelper.NtIconHelper;
...
public class ApplicationPackageManager extends PackageManager {
    ...
    private PermissionManager getPermissionManager() {
        ...
        synchronized (this.mLock) {
            ...
            permissionManager = this.mPermissionManager;
        }
    ...
    public PackageInfo getPackageInfoAsUser(String packageName, int flags, int userId) throws PackageManager.NameNotFoundException {
        PackageInfo pi = getPackageInfoAsUserCached(packageName, updateFlagsForPackage(flags, userId), userId);
        if (pi == null) {
    ...
    public void requestChecksums(String packageName, boolean includeSplits, int required, List<Certificate> trustedInstallers, final PackageManager.OnChecksumsReadyListener onChecksumsReadyListener) throws CertificateEncodingException, PackageManager.NameNotFoundException {
        ...
        try {
            ...
            this.mPM.requestChecksums(packageName, includeSplits, 127, required, encodeCertificates(trustedInstallers), onChecksumsReadyListenerDelegate, getUserId());
    ...
    public Drawable getDrawable(String packageName, int resId, ApplicationInfo appInfo) {
        return getDrawable(packageName, resId, appInfo, false);
    }

    private Drawable getDrawable(String packageName, int resId, ApplicationInfo appInfo, boolean isForceCut) {
        ...
        if (resId != 0) {
            try {
                ...
                Drawable dr = r.getDrawableForDensity(resId, 640, null);
                if (dr == null) {
                    dr = r.getDrawable(resId, null);
                }
                if (appInfo.icon == resId || isForceCut) {
                    dr = Injector.makeThemeIcon(r, dr, appInfo);
                }
                if (dr != null) {
    ...
    public Drawable getUserBadgedIcon(Drawable icon, UserHandle user) {
        ...
        Drawable badge = new LauncherIcons(this.mContext).getBadgeDrawable(getUserManager().getUserIconBadgeResId(user.getIdentifier()), getUserBadgeColor(user, false));
    ...
    public Drawable getUserBadgeForDensity(UserHandle user, int density) {
        ...
        Drawable badgeForeground = getDrawableForDensity(getUserManager().getUserBadgeResId(user.getIdentifier()), density);
    ...
    public Drawable getUserBadgeForDensityNoBackground(UserHandle user, int density) {
        ...
        Drawable badge = getDrawableForDensity(getUserManager().getUserBadgeNoBackgroundResId(user.getIdentifier()), density);
        if (badge != null) {
    ...
    public PackageInfo getPackageArchiveInfo(String archiveFilePath, int flags) {
        ...
        boolean collectCertificates = ((flags & 64) == 0 && (134217728 & flags) == 0) ? false : true;
        ParseInput input = ParseTypeImpl.forParsingWithoutPlatformCompat().reset();
        ParseResult<ParsingPackage> result = ParsingPackageUtils.parseDefault(input, new File(archiveFilePath), 0, getPermissionManager().getSplitPermissions(), collectCertificates);
        if (result.isError()) {
            return null;
        }
        return PackageInfoWithoutStateUtils.generate(result.getResult(), null, flags, 0L, 0L, null, new PackageUserState(), UserHandle.getCallingUserId());
    }
    ...
    public int movePackage(String packageName, VolumeInfo vol) {
        ...
        try {
            if (VolumeInfo.ID_PRIVATE_INTERNAL.equals(vol.id)) {
                ...
                Objects.requireNonNull(volumeUuid2);
    ...
    public Drawable loadUnbadgedItemIcon(PackageItemInfo itemInfo, ApplicationInfo appInfo) {
        ...
        if (itemInfo.packageName != null) {
            dr = getDrawable(itemInfo.packageName, itemInfo.icon, appInfo, itemInfo instanceof ActivityInfo);
        }
    ...
    public static class Injector {
        ...
        static Drawable makeThemeIcon(Resources res, Drawable srcDrawable, ApplicationInfo appInfo) {
            if ("android.content.cts".equals(appInfo.packageName) || "com.google.android.setupwizard".equals(ActivityThread.currentPackageName())) {
                return srcDrawable;
            }
            Drawable dr = NtIconHelper.makeThemeIcon(res, srcDrawable, appInfo);
            return dr;
        }
    }
}
