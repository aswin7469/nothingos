...
import nothing.view.WindowManagerExt;
...
public class Instrumentation {
    ...
    public void sendPointerSync(MotionEvent event) {
        ...
        try {
            WindowManagerGlobal.getWindowManagerService().injectInputAfterTransactionsApplied(event, 2, true);
        } catch (RemoteException e) {
        }
    }
    ...
    public ActivityResult execStartActivity(Context who, IBinder contextThread, IBinder token, Activity target, Intent intent, int requestCode, Bundle options) {
        ...
        try {
            ...
            try {
                Bundle options2 = computeOptions(contextThread, options, target);
                try {
                    ...
                    try {
                        int result2 = service.startActivity(whoThread, opPackageName, attributionTag, intent, resolveTypeIfNeeded, token, str, requestCode, 0, null, options2);
    ...
    public int execStartActivitiesAsUser(Context who, IBinder contextThread, IBinder token, Activity target, Intent[] intents, Bundle options, int userId) {
        ...
        try {
            ...
            try {
                try {
                    int result2 = ActivityTaskManager.getService().startActivities(whoThread, who.getOpPackageName(), who.getAttributionTag(), intents, resolvedTypes, token, computeOptions(contextThread, bundle, target), userId);
    ...
    public ActivityResult execStartActivity(Context who, IBinder contextThread, IBinder token, String target, Intent intent, int requestCode, Bundle options) {
        ...
        try {
            try {
                try {
                    int result2 = ActivityTaskManager.getService().startActivity(whoThread, who.getOpPackageName(), who.getAttributionTag(), intent, intent.resolveTypeIfNeeded(who.getContentResolver()), token, target, requestCode, 0, null, computeOptions(contextThread, bundle, null));
                    checkStartActivityResult(result2, intent);
                    return null;
                } catch (RemoteException e2) {
                    e = e2;
    ...
    public ActivityResult execStartActivity(Context who, IBinder contextThread, IBinder token, String resultWho, Intent intent, int requestCode, Bundle options, UserHandle user) {
        ...
        try {
            ...
            try {
                try {
                    try {
                        int result2 = ActivityTaskManager.getService().startActivityAsUser(whoThread, who.getOpPackageName(), who.getAttributionTag(), intent, intent.resolveTypeIfNeeded(who.getContentResolver()), token, resultWho, requestCode, 0, null, computeOptions(contextThread, bundle, null), user.getIdentifier());
                        try {
                            checkStartActivityResult(result2, intent);
                            return null;
                        } catch (RemoteException e) {
    ...
    public ActivityResult execStartActivityAsCaller(Context who, IBinder contextThread, IBinder token, Activity target, Intent intent, int requestCode, Bundle options, IBinder permissionToken, boolean ignoreTargetSecurity, int userId) {
        ...
        try {
            ...
            try {
                try {
                    try {
                        int result2 = ActivityTaskManager.getService().startActivityAsCaller(whoThread, who.getOpPackageName(), intent, intent.resolveTypeIfNeeded(who.getContentResolver()), token, target != null ? target.mEmbeddedID : null, requestCode, 0, null, computeOptions(contextThread, bundle, target), permissionToken, ignoreTargetSecurity, userId);
                        try {
                            checkStartActivityResult(result2, intent);
                            return null;
                        } catch (RemoteException e) {
    ...
    public void execStartActivityFromAppTask(Context who, IBinder contextThread, IAppTask appTask, Intent intent, Bundle options) {
        ...
        try {
            ...
            intent.prepareToLeaveProcess(who);
            try {
                int result2 = appTask.startActivity(whoThread.asBinder(), who.getOpPackageName(), who.getAttributionTag(), intent, intent.resolveTypeIfNeeded(who.getContentResolver()), computeOptions(contextThread, options, null));
                checkStartActivityResult(result2, intent);
            } catch (RemoteException e2) {
                e = e2;
                throw new RuntimeException("Failure from system", e);
    ...
    private Bundle computeOptions(IBinder contextThread, Bundle options, Activity target) {
        if (options != null) {
            if (options.get(ActivityOptions.KEY_START_WINDOW_MODE) != null) {
                return options;
            }
            if (options.get(ActivityOptions.KEY_START_SPACE_MODE) != null) {
                return options;
            }
        }
        ActivityThread activityThread = this.mThread;
        ...
        Activity target2 = target != null ? target : activityThread.getTopActivity();
        int windowingMode = activityThread.getWindowingMode(target2);
        if (Configuration.isNtWindowMode(windowingMode) && (!target2.mStopped || WindowManagerExt.getInstance(target2).isWindowMode(windowingMode, target2.getActivityToken()))) {
            if (ActivityThread.DEBUG_CONFIGURATION) {
                Log.i(TAG, "Options set window mode.");
            }
            options = options == null ? new Bundle() : options;
            if (windowingMode == 11) {
                options.putBoolean(ActivityOptions.KEY_START_WINDOW_MODE, true);
            } else if (windowingMode == 12) {
                options.putBoolean(ActivityOptions.KEY_START_PIN_WINDOW_MODE, true);
            }
        }
        return options;
    }
