...
public class WindowConfiguration implements Parcelable, Comparable<WindowConfiguration> {
    ...
    public static final Parcelable.Creator<WindowConfiguration> CREATOR = new Parcelable.Creator<WindowConfiguration>() { // from class: android.app.WindowConfiguration.1
        ...
        public WindowConfiguration mo3559createFromParcel(Parcel in) {
        ...
        public WindowConfiguration[] mo3560newArray(int size) {
    ...
    public static final int WINDOWING_MODE_FREEFORM_WINDOW = 11;
    public static final int WINDOWING_MODE_FULLSCREEN = 1;
    public static final int WINDOWING_MODE_MULTI_WINDOW = 6;
    public static final int WINDOWING_MODE_PINNED = 2;
    public static final int WINDOWING_MODE_PINNED_WINDOW = 12;
    ...
    public int compareTo(WindowConfiguration that) {
        ...
        return n16 != 0 ? n16 : this.mDisplayWindowingMode - that.mDisplayWindowingMode;
    }
    ...
    public int hashCode() {
        ...
        return (((((((((((((result * 31) + Objects.hashCode(this.mBounds)) * 31) + Objects.hashCode(this.mMaxBounds)) * 31) + this.mWindowingMode) * 31) + this.mActivityType) * 31) + this.mAlwaysOnTop) * 31) + this.mRotation) * 31) + this.mDisplayWindowingMode;
    }
    ...
    public static boolean isFloating(int windowingMode) {
        return windowingMode == 5 || windowingMode == 2 || isNtWindowMode(windowingMode);
    }

    public static boolean inMultiWindowMode(int windowingMode) {
        return (windowingMode == 1 || windowingMode == 0 || isNtWindowMode(windowingMode)) ? false : true;
    }
    ...
    public boolean isAlwaysOnTop() {
        int i = this.mWindowingMode;
        if ((i == 12 && this.mAlwaysOnTop == 1) || i == 2 || this.mActivityType == 5) {
            return true;
        }
    ...
    public static int composeWindowMode(int... windowModes) {
        int mask = 0;
        int mergeValue = 0;
        for (int windowMode : windowModes) {
            mergeValue += windowMode << mask;
            mask += 4;
        }
        return mergeValue;
    }

    public static boolean containsWindowMode(int windowingModes, int windowingMode) {
        int mask = 15;
        for (int i = 0; i < 8; i++) {
            int value = (windowingModes & mask) >> (i * 4);
            if (value == windowingMode) {
                return true;
            }
            mask <<= 4;
        }
    ...
    public static int[] decomposeWindowMode(int windowingModes) {
        int[] windowModes = new int[8];
        int mask = 15;
        for (int i = 0; i < 8; i++) {
            windowModes[i] = (windowingModes & mask) >> (i * 4);
            mask <<= 4;
        }
        return windowModes;
    }

    public boolean isAlwaysOnTop(boolean onlyDisplay) {
        if (!isAlwaysOnTop()) {
            return onlyDisplay && this.mWindowingMode == 11 && this.mAlwaysOnTop == 1;
        }
        return true;
    }

    public static boolean isNtWindowMode(int windowingMode) {
        return windowingMode == 12 || windowingMode == 11;
    }

    public static boolean isNtPinWindow(int windowingMode) {
        return windowingMode == 12;
    }

    public static boolean isNtMiniWindow(int windowingMode) {
        return windowingMode == 11;
    }
}
