...
public abstract class ContentProvider implements ContentInterface, ComponentCallbacks2 {
    ...
    public class Transport extends ContentProviderNative {
        ...
        public Cursor query(AttributionSource original, Uri uri, String[] projection, Bundle queryArgs, ICancellationSignal cancellationSignal) {
            ...
            if (enforceReadPermission(original2, uri2) == 0) {
        ...
        public void getTypeAsync(Uri uri, RemoteCallback callback) {
            ...
            try {
                result.putString("result", getType(uri));
            } catch (Exception e) {
        ...
        @Override // android.content.IContentProvider
        public Uri insert(AttributionSource original, Uri uri, ContentValues initialValues, Bundle extras) {
            AttributionSource original2;
            ...
            if (enforceWritePermission(original2, uri3) != 0) {
                original2 = ContentProvider.this.setCallingAttributionSource(original2);
                try {
    ...
    boolean checkUser(int pid, int uid, Context context) {
        return UserHandle.getUserId(uid) == context.getUserId() || this.mSingleUser || context.checkPermission(Manifest.permission.INTERACT_ACROSS_USERS, pid, uid) == 0 || context.checkPermission(Manifest.permission.INTERACT_ACROSS_USERS_FULL, pid, uid) == 0;
    }

    private int checkPermission(String permission, AttributionSource attributionSource) {
    ...
    public int enforceWritePermissionInner(Uri uri, AttributionSource attributionSource) throws SecurityException {
        ...
        String failReason = this.mExported ? " requires " + missingPerm2 + ", or grantUriPermission()" : " requires the provider be exported, or grantUriPermission()";
