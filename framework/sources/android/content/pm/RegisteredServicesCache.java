...
public abstract class RegisteredServicesCache<V> {
    ...
    public abstract V mo703parseServiceAttributes(Resources resources, String str, AttributeSet attributeSet);
    ...
    protected ServiceInfo<V> parseServiceInfo(ResolveInfo service) throws XmlPullParserException, IOException {
        ...
        try {
            try {
                ...
                V v = mo703parseServiceAttributes(pm.getResourcesForApplication(si.applicationInfo), si.packageName, attrs);
                if (v == null) {
                    return null;
                }
            ...
            do {
                if (eventType2 == 2 && parser.getDepth() == 2) {
                    ...
                    if ("service".equals(tagName2)) {
                        V service = this.mSerializerAndParser.mo704createFromXml(parser);
                        if (service != null) {
    ...
    private void writePersistentServicesLocked(UserServices<V> user, int userId) {
        ...
        try {
            ...
            for (Map.Entry<V, Integer> service : user.persistentServices.entrySet()) {
                ...
                out.attributeInt(null, GrantCredentialsPermissionActivity.EXTRAS_REQUESTING_UID, service.getValue().intValue());
