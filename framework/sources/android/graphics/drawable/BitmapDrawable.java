...
public class BitmapDrawable extends Drawable {
    ...
    public BitmapDrawable(Resources res, String filepath) {
        ...
        try {
            ...
            try {
                Bitmap bitmap = ImageDecoder.decodeBitmap(ImageDecoder.createSource(res, stream), BitmapDrawable$$ExternalSyntheticLambda0.INSTANCE);
                stream.close();
                init(new BitmapState(bitmap), res);
            } catch (Throwable th) {
                try {
    ...
    public BitmapDrawable(Resources res, InputStream is) {
        ...
        this.mOpticalInsets = Insets.NONE;
        try {
            Bitmap bitmap = ImageDecoder.decodeBitmap(ImageDecoder.createSource(res, is), BitmapDrawable$$ExternalSyntheticLambda1.INSTANCE);
            init(new BitmapState(bitmap), res);
        } catch (Exception e) {
            init(new BitmapState((Bitmap) null), res);
            if (this.mBitmapState.mBitmap == null) {
    ...
    private void updateStateFromTypedArray(TypedArray a, int srcDensityOverride) throws XmlPullParserException {
        ...
        if (srcResId != 0) {
            ...
            try {
                ...
                bitmap = ImageDecoder.decodeBitmap(source, BitmapDrawable$$ExternalSyntheticLambda2.INSTANCE);
                if (is != null) {
                    is.close();
                }
            } catch (Exception e) {
