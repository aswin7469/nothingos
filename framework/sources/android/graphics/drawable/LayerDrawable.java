...
public class LayerDrawable extends Drawable implements Drawable.Callback {
    ...
    public LayerDrawable(LayerState state, Resources res) {
        ...
        LayerState mo1110createConstantState = mo1110createConstantState(state, res);
        this.mLayerState = mo1110createConstantState;
        if (mo1110createConstantState.mNumChildren > 0) {
            ensurePadding();
            refreshPadding();
        }
    }

    /* renamed from: createConstantState */
    LayerState mo1110createConstantState(LayerState state, Resources res) {
    ...
    public Drawable mo3113mutate() {
        if (!this.mMutated && super.mo3113mutate() == this) {
            LayerState mo1110createConstantState = mo1110createConstantState(this.mLayerState, null);
            this.mLayerState = mo1110createConstantState;
            ChildDrawable[] array = mo1110createConstantState.mChildren;
            int N = this.mLayerState.mNumChildren;
            for (int i = 0; i < N; i++) {
                Drawable dr = array[i].mDrawable;
                if (dr != null) {
