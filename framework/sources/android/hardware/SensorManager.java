...
public abstract class SensorManager {
    ...
    public Sensor getDefaultSensor(int type) {
        ...
        if (type == 8 || type == 17 || type == 22 || type == 23 || type == 24 || type == 25 || type == 26 || type == 32 || type == 36) {
            wakeUpSensor = true;
        }
        for (Sensor sensor : l) {
    ...
    public boolean isDynamicSensorDiscoverySupported() {
        List<Sensor> sensors = getSensorList(32);
    ...
    public static float getAltitude(float p0, float p) {
        return (1.0f - ((float) Math.pow(p / p0, 0.19029495120048523d))) * 44330.0f;
    }
    ...
    public static void getAngleChange(float[] angleChange, float[] R, float[] prevR) {
        ...
        float rd1 = (pri0 * ri1) + (pri3 * ri4) + (pri6 * ri7);
        float rd4 = (pri1 * ri1) + (pri4 * ri4) + (pri7 * ri7);
    ...
    public static void getRotationMatrixFromVector(float[] R, float[] rotationVector) {
        ...
        if (rotationVector.length >= 4) {
            ...
            q0 = q02 > 0.0f ? (float) Math.sqrt(q02) : 0.0f;
        }
