...
public class SystemSensorManager extends SensorManager {
    ...
    private static final ArrayList<String> sSensorFastestWhiteList;
    ...
    static {
        ...
        sSensorFastestWhiteList = arrayList;
        arrayList.add("appmanager_sensor_fastest");
        arrayList.add("com.android.sensortest");
        arrayList.add("com.nothing.bsptest");
        arrayList.add("com.nothing.media.camera");
        arrayList.add("com.nothing.scheduledpower");
    }
    ...
    public SystemSensorManager(Context context, Looper mainLooper) {
        ...
        this.mHasHighSamplingRateSensorsPermission = packageManager.checkPermission("android.permission.HIGH_SAMPLING_RATE_SENSORS", appInfo.packageName) != 0 ? false : z;
        int index = 0;
        while (true) {
    ...
    private int getSensorFastestdelayUS(SensorEventListener listener, int delayUs) {
        ...
        int resultdelayUs = delayUs;
        if (delayUs < 20000) {
            boolean isSensorFastestWhiteList = sSensorFastestWhiteList.contains(this.mContext.getPackageName());
            if (fullClassName == null || fullClassName.contains("android.hardware.cts")) {
                resultdelayUs = delayUs;
            } else if (delayUs < 20000 && !isSensorFastestWhiteList) {
                resultdelayUs = 20000;
            } else {
                resultdelayUs = delayUs;
            }
        }
        Log.d("SensorManager", "listenerName:" + fullClassName + ",delayUs:" + delayUs + ",addus:" + resultdelayUs);
        return resultdelayUs;
    }
    ...
    protected boolean registerListenerImpl(SensorEventListener listener, Sensor sensor, int delayUs, Handler handler, int maxBatchReportLatencyUs, int reservedFlags) {
        ...
        if (listener == null || sensor == null) {
            ...
            int delayUs2 = getSensorFastestdelayUS(listener, delayUs);
            synchronized (this.mSensorListeners) {
                ...
                return queue.addSensor(sensor, delayUs2, maxBatchReportLatencyUs);
            }
        }
    }
    ...
    public int configureDirectChannelImpl(SensorDirectChannel channel, Sensor sensor, int rate) {
        ...
        if (sensor != null && isSensorInCappedSet(sensor.getType()) && rate > 1 && this.mIsPackageDebuggable && !this.mHasHighSamplingRateSensorsPermission && Compatibility.isChangeEnabled((long) CHANGE_ID_SAMPLING_RATE_SENSORS_PERMISSION)) {
    ...
    protected SensorDirectChannel createDirectChannelImpl(MemoryFile memoryFile, HardwareBuffer hardwareBuffer) {
        ...
        if (memoryFile != null) {
            ...
            long size3 = hardwareBuffer.getWidth();
    ...
    public static abstract class BaseEventQueue {
        ...
        private int enableSensor(Sensor sensor, int rateUs, int maxBatchReportLatencyUs) {
            ...
            if (sensor != null) {
                if (this.mManager.isSensorInCappedSet(sensor.getType()) && rateUs < 5000 && this.mManager.mIsPackageDebuggable && !this.mManager.mHasHighSamplingRateSensorsPermission && Compatibility.isChangeEnabled((long) SystemSensorManager.CHANGE_ID_SAMPLING_RATE_SENSORS_PERMISSION)) {
