...
public class CaptureResult extends CameraMetadata<Key<?>> {
    ...
    @PublicKey
    public static final Key<Boolean> SENSOR_RAW_BINNING_FACTOR_USED = new Key<>("android.sensor.rawBinningFactorUsed", Boolean.TYPE);
    @PublicKey
    ...
    public CaptureResult(String cameraId, CameraMetadataNative results, CaptureRequest parent, CaptureResultExtras extras) {
        if (results == null) {
    ...
    public CaptureResult(String cameraId, CameraMetadataNative results, CaptureRequest parent, int requestId, long frameNumber) {
        if (results == null) {
            throw new IllegalArgumentException("results was null");
        }
        if (parent == null) {
    ...
    public <T> T get(Key<T> key) {
        T value = (T) this.mResults.get(key);
        return value;
    }
