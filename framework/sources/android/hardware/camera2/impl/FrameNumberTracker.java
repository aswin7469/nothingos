    ...
    private void update() {
        ...
        while (iter.hasNext()) {
            ...
            if (!this.mPendingFrameNumbers[requestType].isEmpty()) {
                if (errorFrameNumber == this.mPendingFrameNumbers[requestType].element()) {
                ...
                while (true) {
                    ...
                    if (this.mPendingFrameNumbersWithOtherType[otherType].isEmpty() || errorFrameNumber != this.mPendingFrameNumbersWithOtherType[otherType].element()) {
                        i++;
                    } else {
    ...
    private void updateCompletedFrameNumber(long frameNumber, int requestType) throws IllegalArgumentException {
        ...
        if (frameNumber < maxOtherFrameNumberSeen) {
            if (!this.mPendingFrameNumbers[requestType].isEmpty()) {
                ...
                boolean inSkippedOther2 = false;
                boolean inSkippedOther1 = index1 != -1;
                if (index2 != -1) {
                    inSkippedOther2 = true;
                }
