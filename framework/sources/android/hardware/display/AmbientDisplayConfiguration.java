...
public class AmbientDisplayConfiguration {
    private static final String TAG = "AODConfig";
    ...
    public boolean enabled(int user) {
        Log.d(TAG, "enabled: pulseOnNotification = " + pulseOnNotificationEnabled(user) + " pulseOnLongPress = " + pulseOnLongPressEnabled(user) + " alwaysOnEnabled = " + alwaysOnEnabled(user) + " wakeLockScreenGestureEnabled(user) = " + wakeLockScreenGestureEnabled(user) + " wakeDisplayGestureEnabled(user) = " + wakeDisplayGestureEnabled(user) + " pickupGestureEnabled(user) = " + pickupGestureEnabled(user) + " tapGestureEnabled(user) = " + tapGestureEnabled(user) + " doubleTapGestureEnabled(user) = " + doubleTapGestureEnabled(user) + " quickPickupSensorEnabled(user) = " + quickPickupSensorEnabled(user) + " screenOffUdfpsEnabled(user) = " + screenOffUdfpsEnabled(user));
    ...
    public boolean screenOffUdfpsEnabled(int user) {
        ...
        if (this.mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_FINGERPRINT)) {
            fingerprintManager = (FingerprintManager) this.mContext.getSystemService(Context.FINGERPRINT_SERVICE);
        }
        Log.d(TAG, "screenOffUdfpsEnabled: fingerprint manager = " + fingerprintManager);
        if (fingerprintManager != null) {
            Log.d(TAG, "screenOffUdfpsEnabled:  hasEnrolledFingerprints = " + fingerprintManager.hasEnrolledFingerprints(user));
            return fingerprintManager.hasEnrolledFingerprints(user);
        }
        return false;
    }
    ...
    public boolean alwaysOnEnabled(int user) {
        Log.d(TAG, "alwaysOnEnabled: doze_always_on = " + boolSetting(Settings.Secure.DOZE_ALWAYS_ON, user, this.mAlwaysOnByDefault ? 1 : 0) + " alwayOnAvailable = " + alwaysOnAvailable() + " accessibilityInversionEnabled = " + accessibilityInversionEnabled(user));
