...
public final class SoundTriggerDetector {
    ...
    public SoundTriggerDetector(ISoundTriggerSession soundTriggerSession, UUID soundModelId, Callback callback, Handler handler) {
    ...
    public boolean startRecognition(int recognitionFlags) {
        ...
        try {
            int status = this.mSoundTriggerSession.startRecognition(new ParcelUuid(this.mSoundModelId), this.mRecognitionCallback, new SoundTrigger.RecognitionConfig(captureTriggerAudio, allowMultipleTriggers, null, null, audioCapabilities), runInBatterySaver);
            return status == 0;
    ...
    public boolean stopRecognition() {
        try {
            int status = this.mSoundTriggerSession.stopRecognition(new ParcelUuid(this.mSoundModelId), this.mRecognitionCallback);
            return status == 0;
        } catch (RemoteException e) {
            return false;
        }
