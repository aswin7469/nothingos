...
public class GLLogWrapper extends GLWrapperBase {
            ...
            for (int i = 0; i < size; i++) {
                ...
                switch (type) {
                    case 5120:
                        ...
                        break;
                    case 5121:
                        byte d2 = byteBuffer.get(byteOffset);
                        builder.append(Integer.toString(d2 & 255));
                        break;
                    case 5122:
                        ShortBuffer shortBuffer = byteBuffer.asShortBuffer();
                        short d3 = shortBuffer.get(byteOffset / 2);
                        builder.append(Integer.toString(d3));
                        break;
                    case 5126:
                        FloatBuffer intBuffer = byteBuffer.asFloatBuffer();
                        float d4 = intBuffer.get(byteOffset / 4);
                        builder.append(Float.toString(d4));
                        break;
                    case 5132:
                        IntBuffer intBuffer2 = byteBuffer.asIntBuffer();
                        int d5 = intBuffer2.get(byteOffset / 4);
                        builder.append(Integer.toString(d5));
                        break;
                    default:
                        builder.append("?");
                        break;
    ...
    public void glCompressedTexSubImage2D(int target, int level, int xoffset, int yoffset, int width, int height, int format, int imageSize, Buffer data) {
        ...
        arg("width", width);
        arg("height", height);
        arg(Telephony.CellBroadcasts.MESSAGE_FORMAT, format);
        arg("imageSize", imageSize);
        arg("data", data.toString());
        end();
