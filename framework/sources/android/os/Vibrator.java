...
public abstract class Vibrator {
    ...
    public final void vibrate(int uid, String opPkg, VibrationEffect vibe, String reason, AudioAttributes attributes) {
        ...
        if (checkDoAACVibrationInstead(attributes, uid, opPkg, vibe, reason)) {
            return;
        }
    ...
    /* JADX WARN: Type inference failed for: r13v0, types: [android.os.Vibrator$1] */
    private boolean checkDoAACVibrationInstead(final AudioAttributes attributes, final int uid, final String opPkg, final VibrationEffect vibe, final String reason) {
        if ("com.google.android.deskclock".equals(opPkg)) {
            Context context = ActivityThread.currentActivityThread().getSystemContext();
            final AudioManager audioManager = (AudioManager) context.getSystemService("audio");
            final int usage = attributes.getUsage();
            if (usage == 4 || System.currentTimeMillis() - audioManager.getEnableDoVibrateTime(opPkg) < 3500) {
                audioManager.setEnableDoVibrateTime(opPkg, System.currentTimeMillis());
                if (usage == 4) {
                    new Thread() { // from class: android.os.Vibrator.1
                        @Override // java.lang.Thread, java.lang.Runnable
                        public void run() {
                            try {
                                Thread.sleep(200L);
                                if (audioManager.getEnableDoVibrateTime(opPkg) > System.currentTimeMillis()) {
                                    Log.d(Vibrator.TAG, "[ALARM_VIB] checkDoAACVibrationInstead: delay intercept alarm vib for usage_" + usage);
                                    return;
                                }
                                VibrationAttributes attr = new VibrationAttributes.Builder(attributes, vibe).build();
                                Vibrator.this.vibrate(uid, opPkg, vibe, reason, attr);
                ...
                Log.d(TAG, "[ALARM_VIB] checkDoAACVibrationInstead: intercept alarm vib for usage_" + usage);
                return true;
            }
            return false;
        }
