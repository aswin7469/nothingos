...
public final class IncrementalStorage {
    ...
    public void makeFile(String path, long size, UUID id, byte[] metadata, byte[] v4signatureBytes, byte[] content) throws IOException {
        try {
            ...
            int res = this.mService.makeFile(this.mId, path, params, content);
            if (res != 0) {
    ...
    private static void validateV4Signature(byte[] v4signatureBytes) throws IOException {
        ...
        try {
            ...
            } else if (hashingInfo.log2BlockSize != 12) {
                throw new IOException("Unsupported log2BlockSize: " + ((int) hashingInfo.log2BlockSize));
            } else if (hashingInfo.salt != null && hashingInfo.salt.length > 0) {
                throw new IOException("Unsupported salt: " + hashingInfo.salt);
            } else if (hashingInfo.rawRootHash.length != 32) {
