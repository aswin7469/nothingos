...
public class SeekBarVolumizer implements SeekBar.OnSeekBarChangeListener, Handler.Callback {
    ...
    public SeekBarVolumizer(Context context, int streamType, Uri defaultUri, Callback callback, boolean playSample) {
        ...
        this.mMuted = audioManager.isStreamMute(streamType);
        this.mPlaySample = playSample;
        if (callback != null) {
            boolean mute = this.mMuted;
            if (streamType == 3) {
                mute = this.mOriginalStreamVolume == 0 ? true : z;
            }
            callback.onMuted(mute, isZenMuted());
        }
        if (defaultUri == null) {
            if (streamType == 2) {
    ...
    public int getVolumeGroupIdForLegacyStreamType(int streamType) {
        ...
        return ((Integer) AudioManager.getAudioProductStrategies().stream().map(SeekBarVolumizer$$ExternalSyntheticLambda0.INSTANCE).filter(SeekBarVolumizer$$ExternalSyntheticLambda1.INSTANCE).findFirst().orElse(-1)).intValue();
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (AudioManager.VOLUME_CHANGED_ACTION.equals(action)) {
                ...
                if (SeekBarVolumizer.this.mNotificationOrRing) {
                    ...
                    seekBarVolumizer2.mRingerMode = seekBarVolumizer2.mAudioManager.getRingerModeInternal();
                }
                ...
                seekBarVolumizer3.mZenMode = seekBarVolumizer3.mNotificationManager.getZenMode();
                SeekBarVolumizer.this.updateSlider();
                ...
                seekBarVolumizer4.mNotificationPolicy = seekBarVolumizer4.mNotificationManager.getConsolidatedNotificationPolicy();
                SeekBarVolumizer seekBarVolumizer5 = SeekBarVolumizer.this;
                boolean z = false;
                seekBarVolumizer5.mAllowAlarms = (seekBarVolumizer5.mNotificationPolicy.priorityCategories & 32) != 0;
                SeekBarVolumizer seekBarVolumizer6 = SeekBarVolumizer.this;
                if ((seekBarVolumizer6.mNotificationPolicy.priorityCategories & 64) != 0) {
                    z = true;
                }
                seekBarVolumizer6.mAllowMedia = z;
                SeekBarVolumizer.this.mAllowRinger = !ZenModeConfig.areAllPriorityOnlyRingerSoundsMuted(seekBarVolumizer.mNotificationPolicy);
                SeekBarVolumizer.this.updateSlider();
            }
        }
    ...
    private void registerVolumeGroupCb() {
        if (this.mVolumeGroupId != -1) {
            this.mAudioManager.registerVolumeGroupCallback(MediaRouter2$$ExternalSyntheticLambda8.INSTANCE, this.mVolumeGroupCallback);
            updateSlider();
        }
    }
