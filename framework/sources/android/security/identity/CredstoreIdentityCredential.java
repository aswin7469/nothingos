...
class CredstoreIdentityCredential extends IdentityCredential {
    ...
    public CredstoreIdentityCredential(Context context, String credentialName, int cipherSuite, ICredential binder) {
    ...
    public long getCredstoreOperationHandle() {
        if (!this.mOperationHandleSet) {
            try {
                this.mOperationHandle = this.mBinder.selectAuthKey(this.mAllowUsingExhaustedKeys, this.mAllowUsingExpiredKeys);
    ...
    public ResultData getEntries(byte[] requestMessage, Map<String, Collection<String>> entriesToRequest, byte[] sessionTranscript, byte[] readerSignature) throws SessionTranscriptMismatchException, NoAuthenticationKeyAvailableException, InvalidReaderSignatureException, EphemeralPublicKeyNotFoundException, InvalidRequestMessageException {
        ...
        try {
            int i = 0;
            GetEntriesResultParcel resultParcel2 = this.mBinder.getEntries(requestMessage != null ? requestMessage : new byte[0], rnsParcels, sessionTranscript != null ? sessionTranscript : new byte[0], readerSignature != null ? readerSignature : new byte[0], this.mAllowUsingExhaustedKeys, this.mAllowUsingExpiredKeys);
            byte[] mac = resultParcel2.mac;
            if (mac != null && mac.length == 0) {
                mac = null;
            }
            CredstoreResultData.Builder resultDataBuilder = new CredstoreResultData.Builder(resultParcel2.staticAuthenticationData, resultParcel2.deviceNameSpaces, mac);
    ...
    public void setAvailableAuthenticationKeys(int keyCount, int maxUsesPerKey) {
        try {
            this.mBinder.setAvailableAuthenticationKeys(keyCount, maxUsesPerKey);
        } catch (RemoteException e) {
            throw new RuntimeException("Unexpected RemoteException ", e);
        } catch (ServiceSpecificException e2) {
            throw new RuntimeException("Unexpected ServiceSpecificException with code " + e2.errorCode, e2);
