...
public final class FillEventHistory implements Parcelable {
    public static final Parcelable.Creator<FillEventHistory> CREATOR = new Parcelable.Creator<FillEventHistory>() { // from class: android.service.autofill.FillEventHistory.1
        ...
        public FillEventHistory mo3559createFromParcel(Parcel parcel) {
            ...
            while (i < numEvents) {
                ...
                selection.addEvent(new Event(eventType, datasetId, clientState, selectedDatasetIds, readArraySet, changedFieldIds, changedDatasetIds, manuallyFilledFieldIds, manuallyFilledDatasetIds, detectedFieldIds, detectedFieldClassifications, saveDialogNotShowReason));
                i++;
                parcel2 = parcel;
            }
            return selection;
        ...
        public FillEventHistory[] mo3560newArray(int size) {
    ...
    public static final class Event {
        ...
        public Event(int eventType, String datasetId, Bundle clientState, List<String> selectedDatasetIds, ArraySet<String> ignoredDatasetIds, ArrayList<AutofillId> changedFieldIds, ArrayList<String> changedDatasetIds, ArrayList<AutofillId> manuallyFilledFieldIds, ArrayList<ArrayList<String>> manuallyFilledDatasetIds, AutofillId[] detectedFieldIds, FieldClassification[] detectedFieldClassifications, int saveDialogNotShowReason) {
            boolean z;
            this.mEventType = Preconditions.checkArgumentInRange(eventType, 0, 5, "eventType");
            ...
            if (changedFieldIds != null) {
                if (ArrayUtils.isEmpty(changedFieldIds) || changedDatasetIds == null || changedFieldIds.size() != changedDatasetIds.size()) {
                    z = false;
                } else {
                    z = true;
                }
                Preconditions.checkArgument(z, "changed ids must have same length and not be empty");
            }
            this.mChangedFieldIds = changedFieldIds;
            this.mChangedDatasetIds = changedDatasetIds;
            if (manuallyFilledFieldIds != null) {
                Preconditions.checkArgument((ArrayUtils.isEmpty(manuallyFilledFieldIds) || manuallyFilledDatasetIds == null || manuallyFilledFieldIds.size() != manuallyFilledDatasetIds.size()) ? false : z2, "manually filled ids must have same length and not be empty");
            }
