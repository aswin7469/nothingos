...
public final class FillWindow implements AutoCloseable {
        ...
        synchronized (this.mLock) {
            if (this.mWm != null && this.mFillView != null) {
                try {
                    ...
                    if (AugmentedAutofillService.sDebug) {
                        Log.d(TAG, "Filed with token " + p.token + " gone.");
                    }
    ...
    public static final class FillWindowPresenter extends IAutofillWindowPresenter.Stub {
        ...
        public void show(WindowManager.LayoutParams p, Rect transitionEpicenter, boolean fitsSystemWindows, int layoutDirection) {
            ...
            if (fillWindow != null) {
                fillWindow.mUiThreadHandler.sendMessage(PooledLambda.obtainMessage(FillWindow$FillWindowPresenter$$ExternalSyntheticLambda0.INSTANCE, fillWindow, p));
            }
        }
        ...
        public void hide(Rect transitionEpicenter) {
            ...
            FillWindow fillWindow = this.mFillWindowReference.get();
            if (fillWindow != null) {
                fillWindow.mUiThreadHandler.sendMessage(PooledLambda.obtainMessage(FillWindow$FillWindowPresenter$$ExternalSyntheticLambda1.INSTANCE, fillWindow));
            }
        }
    }
}
