...
public final class TranslationServiceInfo {
    ...
    private TranslationServiceInfo(Context context, ServiceInfo si) {
        if (!Manifest.permission.BIND_TRANSLATION_SERVICE.equals(si.permission)) {
            Slog.w(TAG, "TranslationServiceInfo from '" + si.packageName + "' does not require permission " + Manifest.permission.BIND_TRANSLATION_SERVICE);
        ...
        try {
            ...
            if (XML_TAG_SERVICE.equals(parser.getName())) {
                ...
                TypedArray afsAttributes = resources.obtainAttributes(allAttributes, R.styleable.TranslationService);
