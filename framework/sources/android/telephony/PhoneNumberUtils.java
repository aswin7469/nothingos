    ...
    */
    ...
    public static void formatNanpNumber(Editable text) {
            ...
            switch (c) {
                case '+':
                    if (i == 0) {
                        ...
                        text.mo2736replace(0, length2, saved);
                        return;
                    }
                case ',':
                case '.':
        ...
        for (int i2 = 0; i2 < numDashes; i2++) {
            ...
            text.mo2736replace(pos + i2, pos + i2, NativeLibraryHelper.CLEAR_ABI_OVERRIDE);
        }
        for (int len = text.length(); len > 0 && text.charAt(len - 1) == '-'; len--) {
            text.mo2733delete(len - 1, len);
        }
    }
    ...
    public static String getUsernameFromUriNumber(String number) {
        ...
        if (delimiterIndex < 0) {
            com.android.telephony.Rlog.w(LOG_TAG, "getUsernameFromUriNumber: no delimiter found in SIP addr '" + number + "'");
    ...
    public static boolean areSamePhoneNumber(String number1, String number2, String defaultCountryIso) {
        PhoneNumberUtil util = PhoneNumberUtil.getInstance();
        String defaultCountryIso2 = defaultCountryIso.toUpperCase();
        try {
