...
public final class RcsConfig {
        ...
        try {
            try {
                ...
                while (eventType != 1 && current != null) {
                    if (eventType == 2) {
                        String tag = xpp.getName().trim().toLowerCase();
                        if (TAG_CHARACTERISTIC.equals(tag)) {
                            ...
                            if (count > 0) {
                                ...
                                while (true) {
                                    ...
                                    if (!"type".equals(name)) {
                                        ...
                                        type = xpp.getAttributeValue(xpp.getAttributeNamespace(i), name).trim().toLowerCase();
                                        break;
                                    }
                                }
                            }
                            ...
                            if (count2 > 1) {
                                for (int i2 = 0; i2 < count2; i2++) {
                                    String name2 = xpp.getAttributeName(i2).trim().toLowerCase();
                                    if ("name".equals(name2)) {
                                        key = xpp.getAttributeValue(xpp.getAttributeNamespace(i2), name2).trim().toLowerCase();
                                    } else if ("value".equals(name2)) {
                                        value = xpp.getAttributeValue(xpp.getAttributeNamespace(i2), name2).trim();
                                    }
                                }
                            }
                        ...
                        current = TAG_CHARACTERISTIC.equals(xpp.getName().trim().toLowerCase()) ? current.getParent() : current;
                    }
                    eventType = xpp.next();
    ...
    public static void updateConfigForSub(Context cxt, int subId, byte[] config, boolean isCompressed) {
        ...
        contentResolver.update(uri, values, "_id=" + subId, null);
    }
    ...
    */
    public static byte[] loadRcsConfigForSub(Context cxt, int subId, boolean isCompressed) {
        ...
        ContentResolver contentResolver = cxt.getContentResolver();
        Uri uri = Telephony.SimInfo.CONTENT_URI;
        Cursor cursor = contentResolver.query(uri, null, "_id=" + subId, null, null);
        try {
            if (cursor != null) {
                try {
