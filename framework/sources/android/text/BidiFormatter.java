...
public final class BidiFormatter {
    ...
    public CharSequence unicodeWrap(CharSequence str, TextDirectionHeuristic heuristic, boolean isolate) {
        ...
        if (getStereoReset() && isolate) {
            result.mo2731append((CharSequence) markBefore(str, isRtl ? TextDirectionHeuristics.RTL : TextDirectionHeuristics.LTR));
        }
        if (isRtl != this.mIsRtlContext) {
            result.mo2730append(isRtl ? RLE : LRE);
            result.mo2731append(str);
            result.mo2730append(PDF);
        } else {
            result.mo2731append(str);
        }
        if (isolate) {
            result.mo2731append((CharSequence) markAfter(str, isRtl ? TextDirectionHeuristics.RTL : TextDirectionHeuristics.LTR));
        }
        return result;
    }
        ...
        private byte skipTagForward() {
            ...
            while (true) {
                ...
                if (i < this.length) {
                    ...
                    this.lastChar = '<';
                    return IGnssVisibilityControlCallback.NfwRequestor.OTHER_CHIPSET_VENDOR;
                }
            }
        }
