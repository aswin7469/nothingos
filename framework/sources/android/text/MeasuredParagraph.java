...
public class MeasuredParagraph {
    ...
    public static MeasuredParagraph buildForMeasurement(TextPaint paint, CharSequence text, int start, int end, TextDirectionHeuristic textDir, MeasuredParagraph recycle) {
        ...
        if (mt.mSpanned == null) {
            ...
            while (spanStart < end) {
                ...
                mt.applyMetricsAffectingSpan(paint, (MetricAffectingSpan[]) TextUtils.removeEmptySpans(spans, mt.mSpanned, MetricAffectingSpan.class), spanStart, spanEnd, null);
                spanStart = spanEnd;
            }
        }
        return mt;
    ...
    public static MeasuredParagraph buildForStaticLayout(TextPaint paint, CharSequence text, int start, int end, TextDirectionHeuristic textDir, boolean computeHyphenation, boolean computeLayout, MeasuredParagraph hint, MeasuredParagraph recycle) {
        MeasuredParagraph mt = recycle == null ? obtain() : recycle;
        mt.resetAndAnalyzeBidi(text, start, end, textDir);
        MeasuredText.Builder builder = hint == null ? new MeasuredText.Builder(mt.mCopiedBuffer).setComputeHyphenation(computeHyphenation).setComputeLayout(computeLayout) : new MeasuredText.Builder(hint.mMeasuredText);
        if (mt.mTextLength == 0) {
            ...
            if (mt.mSpanned == null) {
                ...
                while (spanStart < end) {
                    int spanEnd = mt.mSpanned.nextSpanTransition(spanStart, end, MetricAffectingSpan.class);
                    MetricAffectingSpan[] spans = (MetricAffectingSpan[]) mt.mSpanned.getSpans(spanStart, spanEnd, MetricAffectingSpan.class);
                    mt.applyMetricsAffectingSpan(paint, (MetricAffectingSpan[]) TextUtils.removeEmptySpans(spans, mt.mSpanned, MetricAffectingSpan.class), spanStart, spanEnd, builder);
    ...
    private void applyReplacementRun(ReplacementSpan replacement, int start, int end, MeasuredText.Builder builder) {
        ...
        float width = replacement.getSize(textPaint, spanned, start + i, end + i, this.mCachedFm);
        if (builder == null) {
