...
public class TextUtils {
    ...
    public static final Parcelable.Creator<CharSequence> CHAR_SEQUENCE_CREATOR = new Parcelable.Creator<CharSequence>() { // from class: android.text.TextUtils.1
        ...
        public CharSequence mo3559createFromParcel(Parcel p) {
            int kind = p.readInt();
        ...
        public CharSequence[] mo3560newArray(int size) {
        ...
        if (cs instanceof Spanned) {
            ...
            for (int i = 0; i < os.length; i++) {
                ...
                if (prop instanceof CharacterStyle) {
                    prop = ((CharacterStyle) prop).mo2745getUnderlying();
                }
    ...
    public static CharSequence replace(CharSequence template, String[] sources, CharSequence[] destinations) {
        ...
        for (int i2 = 0; i2 < sources.length; i2++) {
            ...
            if (start >= 0) {
                tb.mo2736replace(start, end, destinations[i2]);
            }
        }
        return tb;
    }
                ...
                if (ssb.charAt(i) == '^') {
                    ...
                    if (next == '^') {
                        ...
                        if (which < 0) {
                            ...
                            ssb.mo2736replace(i, i + 2, values[which]);
    ...
    */
        ...
        try {
            ...
            try {
                ...
                if (remaining == 0) {
                    ...
                    ssb.mo2732append(text, 0, left2);
                    ssb.mo2731append((CharSequence) ellipsis);
                    ssb.mo2732append(text, right4, len);
                    if (mt != null) {
                        mt.recycle();
                    }
                    return ssb;
    ...
    public static CharSequence listEllipsize(Context context, List<CharSequence> elements, String separator, TextPaint paint, float avail, int moreId) {
        ...
        for (int i = 0; i < totalLen; i++) {
            output.mo2731append(bidiFormatter.unicodeWrap(elements.get(i)));
            if (i != totalLen - 1) {
                output.mo2731append((CharSequence) separator);
            }
            endIndexes[i] = output.length();
        }
        for (int i2 = totalLen - 1; i2 >= 0; i2--) {
            output.mo2733delete(endIndexes[i2], output.length());
            ...
            if (remainingElements > 0) {
                ...
                output.mo2731append(bidiFormatter.unicodeWrap(morePiece));
            }
    ...
    public static CharSequence concat(CharSequence... text) {
        ...
        if (spanned) {
            ...
            while (i < length2) {
                ...
                ssb.mo2731append(piece == null ? "null" : piece);
                i++;
            }
    ...
    public static String formatSimple(String format, Object... args) {
        ...
        while (i2 < sb.length()) {
            if (sb.charAt(i2) == '%') {
                ...
                sb.replace(i2, i2 + consume, repl);
                if (prefixChar != '0' || repl.charAt(0) != '-') {
                    i = 0;
                }
                int prefixInsert = i;
                for (int k = repl.length(); k < prefixLen; k++) {
                    sb.insert(i2 + prefixInsert, prefixChar);
                }
                int k2 = repl.length();
                i2 += Math.max(k2, prefixLen);
            } else {
                i2++;
            }
        }
