...
public class ApkSignatureVerifier {
    ...
    private static SigningDetailsWithDigests verifyV1Signature(final String apkPath, boolean verifyFull) throws PackageParser.PackageParserException {
        ...
        try {
            try {
                try {
                    ...
                    BoostFramework boostFramework3 = sPerfBoost;
                    if (boostFramework3 != null && !sIsPerfLockAcquired && verifyFull) {
                        boostFramework3.perfHint(BoostFramework.VENDOR_HINT_PACKAGE_INSTALL_BOOST, null, Integer.MAX_VALUE, -1);
                    ...
                    if (verifyFull) {
                        ...
                        C1VerificationData vData = new C1VerificationData();
                        ...
                        for (final ZipEntry entry2 : toVerify) {
                            ...
                            final C1VerificationData c1VerificationData = vData;
                            ZipEntry manifestEntry2 = manifestEntry;
                            List<ZipEntry> toVerify2 = toVerify;
                            C1VerificationData vData2 = vData;
                                    ...
                                    try {
                                        ...
                                        synchronized (strictJarFiles) {
                                            ...
                                            if (tempJarFile == null) {
                                                ...
                                                C1VerificationData c1VerificationData2 = C1VerificationData.this;
                                                int i4 = c1VerificationData2.index;
                                                c1VerificationData2.index = i4 + 1;
                            ...
                            synchronized (vData2.objWaitAll) {
                                if (vData2.exceptionFlag == 0) {
                        ...
                        lastSigs = lastSigs2;
                        C1VerificationData vData3 = vData;
                        map = null;
                        vData3.wait = true;
                        verificationExecutor3.shutdown();
                        while (vData3.wait) {
                            try {
                                if (vData3.exceptionFlag != 0 && !vData3.shutDown) {
                                    Slog.w(TAG, "verifyV1 Exception " + vData3.exceptionFlag);
                                    verificationExecutor3.shutdownNow();
                                    vData3.shutDown = true;
                                }
                                vData3.wait = !verificationExecutor3.awaitTermination(50L, TimeUnit.MILLISECONDS);
                            } catch (InterruptedException e) {
                                Slog.w(TAG, "VerifyV1 interrupted while awaiting all threads done...");
                            }
                        }
                        if (vData3.exceptionFlag != 0) {
                            throw new PackageParser.PackageParserException(vData3.exceptionFlag, "Failed to collect certificates from " + apkPath, vData3.exception);
                        }
                    } else {
                        lastSigs = lastSigs2;
                        map = null;
                    }
                    SigningDetailsWithDigests signingDetailsWithDigests = new SigningDetailsWithDigests(new PackageParser.SigningDetails(lastSigs, 1), map);
                    if (sIsPerfLockAcquired && (boostFramework2 = sPerfBoost) != null) {
                ...
                throw new PackageParser.PackageParserException(PackageManager.INSTALL_PARSE_FAILED_CERTIFICATE_ENCODING, "Failed to collect certificates from " + apkPath, e3);
            }
        } catch (Throwable th) {
