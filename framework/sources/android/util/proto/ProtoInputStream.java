...
public final class ProtoInputStream extends ProtoStream {
    ...
    public long start(long fieldId) throws IOException {
        ...
        if (i == this.mExpectedObjectTokenStack.size()) {
            this.mExpectedObjectTokenStack.add(makeToken(0, (fieldId & ProtoStream.FIELD_COUNT_REPEATED) == ProtoStream.FIELD_COUNT_REPEATED, this.mDepth, (int) fieldId, getOffset() + messageSize));
        } else {
            LongArray longArray = this.mExpectedObjectTokenStack;
            int i2 = this.mDepth;
            longArray.set(i2, makeToken(0, (fieldId & ProtoStream.FIELD_COUNT_REPEATED) == ProtoStream.FIELD_COUNT_REPEATED, i2, (int) fieldId, getOffset() + messageSize));
        }
        int i3 = this.mDepth;
