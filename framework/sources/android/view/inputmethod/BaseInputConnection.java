...
public class BaseInputConnection implements InputConnection {
    ...
    public boolean deleteSurroundingTextInCodePoints(int beforeLength, int afterLength) {
        ...
        if (a >= 0 && b >= 0 && (start = findIndexBackward(content, a, Math.max(beforeLength, 0))) != INVALID_INDEX && (end = findIndexForward(content, b, Math.max(afterLength, 0))) != INVALID_INDEX) {
            ...
            if (numDeleteAfter > 0) {
                content.mo2733delete(b - numDeleteBefore, end - numDeleteBefore);
            }
        }
        endBatchEdit();
        return true;
    ...
    private void ensureDefaultComposingSpans() {
        ...
        if (this.mDefaultComposingSpans == null) {
            ...
            if (view != null) {
                ...
                View servedView = this.mIMM.mCurRootView.getImeFocusController().getServedView();
    ...
    private void replaceText(CharSequence text, int newCursorPosition, boolean composing) {
        ...
        content.mo2736replace(a, b, text);
        endBatchEdit();
    }
