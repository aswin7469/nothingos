...
public class SpellChecker implements SpellCheckerSession.SpellCheckerSessionListener {
    ...
    public void resetSession() {
        ...
        textView.removeMisspelledSpans((Editable) textView.mo3084getText());
    }
    ...
    public void spellCheck(int start, int end, boolean forceCheckWhenEditingWord) {
        ...
        if (locale2 == null || (locale = this.mCurrentLocale) == null || !locale.equals(locale2)) {
            setLocale(locale2);
            start = 0;
            end = this.mTextView.mo3084getText().length();
        } else {
    ...
    private SpellCheckSpan onGetSuggestionsInternal(SuggestionsInfo suggestionsInfo, int offset, int length) {
        ...
        for (int k = 0; k < this.mLength; k++) {
            if (sequenceNumber == this.mIds[k]) {
                ...
                boolean looksLikeGrammarError = z;
    ...
    private static void removeErrorSuggestionSpan(Editable editable, int start, int end, RemoveReason reason) {
    ...
    public class SpellParser {
        ...
        public void parse(int start, int end, boolean forceCheckWhenEditingWord) {
            ...
            if (parseEnd > start) {
                setRangeSpan((Editable) SpellChecker.this.mTextView.mo3084getText(), start, parseEnd);
                parse();
            }
        }

        public boolean isFinished() {
            return ((Editable) SpellChecker.this.mTextView.mo3084getText()).getSpanStart(this.mRange) < 0;
        }

        public void stop() {
            removeRangeSpan((Editable) SpellChecker.this.mTextView.mo3084getText());
        ...
        public void parse() {
            Editable editable = (Editable) SpellChecker.this.mTextView.mo3084getText();
            ...
            if (spellCheckEnd <= spellCheckStart) {
                String str = SpellChecker.TAG;
                Log.w(str, "Trying to spellcheck invalid region, from " + sentenceStart + " to " + spellCheckEnd);
