...
public class SmallSortedMap<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        if (size == i) {
            ...
            getOverflowEntriesMutable().put((K) lastEntryInArray.mo3254getKey(), lastEntryInArray.getValue());
        }
    ...
    private int binarySearchInArray(K key) {
        ...
        if (right >= 0) {
            int cmp = key.compareTo(this.entryList.get(right).mo3254getKey());
            if (cmp > 0) {
                return -(right + 2);
            }
            if (cmp == 0) {
        ...
        while (left <= right) {
            ...
            int cmp2 = key.compareTo(this.entryList.get(mid).mo3254getKey());
            if (cmp2 < 0) {
                right = mid - 1;
            } else if (cmp2 > 0) {
                left = mid + 1;
    ...
    public class Entry implements Map.Entry<K, V>, Comparable<SmallSortedMap<K, V>.Entry> {
        ...
        public int compareTo(SmallSortedMap<K, V>.Entry other) {
            return mo3254getKey().compareTo(other.mo3254getKey());
        }
