...
public final class WSMeansQuantizer implements Quantizer {
    ...
    public WSMeansQuantizer(int[] inClusters, PointProvider pointProvider, Map<Integer, Integer> inputPixelToCount) {
        ...
        this.mClusters = (float[][]) Array.newInstance(float.class, inClusters.length, 3);
        int index = 0;
    ...
    public void quantize(int[] pixels, int maxColors) {
        ...
        this.mPoints = (float[][]) Array.newInstance(float.class, this.mInputPixelToCount.size(), 3);
    ...
    void calculateClusterDistances(int maxColors) {
        if (this.mDistanceMatrix.length != maxColors) {
            this.mDistanceMatrix = (float[][]) Array.newInstance(float.class, maxColors, maxColors);
        }
        ...
        if (this.mIndexMatrix.length != maxColors) {
            this.mIndexMatrix = (int[][]) Array.newInstance(int.class, maxColors, maxColors);
        }
        for (int i2 = 0; i2 < maxColors; i2++) {
            ...
            distances.sort(WSMeansQuantizer$$ExternalSyntheticLambda0.INSTANCE);
    ...
    void recalculateClusterCenters(int maxColors) {
        ...
        for (int i2 = 0; i2 < maxColors; i2++) {
            ...
            float bSum = bSums[i2];
            float cSum = cSums[i2];
            float[][] fArr2 = this.mClusters;
            fArr2[i2][0] = aSum / count2;
            fArr2[i2][1] = bSum / count2;
            fArr2[i2][2] = cSum / count2;
        }
    }
