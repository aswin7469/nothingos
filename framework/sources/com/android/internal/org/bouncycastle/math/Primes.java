        ...
        for (int i = 0; i < iterations; i++) {
            ...
            if (!z.equals(bigInteger) && !z.equals(wSubOne)) {
                ...
                if (!primeToBase) {
                    ...
                    return g2.compareTo(bigInteger2) > 0 ? MROutput.provablyCompositeWithFactor(g2) : MROutput.provablyCompositeNotPrimePower();
                }
            }
        }
        return MROutput.probablyPrime();
    ...
    private static STOutput implSTRandomPrime(Digest d, int length, byte[] primeSeed) {
        ...
        if (length < 33) {
            ...
            do {
                ...
                inc(primeSeed, 1);
                int c2 = extract32(c0) ^ extract32(c1);
                primeGenCounter++;
                long c64 = ((c2 & ((-1) >>> (32 - length))) | (1 << (length - 1)) | 1) & 4294967295L;
                if (isPrime32(c64)) {
        ...
        STOutput rec = implSTRandomPrime(digest, (length + 3) / 2, primeSeed);
        ...
        BigInteger c3 = tx2.multiply(c02).add(bigInteger);
        int dt = 0;
        while (true) {
            int dt2 = dLen;
            if (c3.bitLength() <= length) {
                x = x3;
                BigInteger c4 = c3;
                i = 1;
                c = c4;
                ...
                c = tx2.multiply(c02).add(bigInteger2);
            }
            primeGenCounter2 += i;
            if (!implHasAnySmallFactors(c)) {
                BigInteger a = hashGen(digest, primeSeed2, iterations + 1);
                BigInteger a2 = a.mod(c.subtract(THREE)).add(TWO);
                BigInteger tx22 = tx2.add(BigInteger.valueOf(dt));
                dt = 0;
                BigInteger z = a2.modPow(tx22, c);
                BigInteger bigInteger3 = ONE;
                BigInteger a3 = z.subtract(bigInteger3);
                if (c.gcd(a3).equals(bigInteger3) && z.modPow(c02, c).equals(bigInteger3)) {
