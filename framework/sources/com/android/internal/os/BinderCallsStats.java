...
public class BinderCallsStats implements BinderInternal.Observer {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            if (uidEntry != null) {
                                ...
                                for (int j = 0; j < csize; j++) {
                                    tmpCallStats.add(callStats.valueAt(j).m3423clone());
                                }
    ...
    private void resolveBinderMethodNames(ArrayList<ExportedCallStat> resultCallStats) {
        ...
        resultCallStats.sort(BinderCallsStats$$ExternalSyntheticLambda1.INSTANCE);
    ...
    private void dumpLocked(PrintWriter pw, AppIdToPackageMap packageMap, int workSourceUid, boolean verbose) {
        ...
        exportedCallStats.sort(BinderCallsStats$$ExternalSyntheticLambda2.INSTANCE);
        for (ExportedCallStat e : exportedCallStats) {
            if (e.methodName == null || !e.methodName.startsWith("__DEBUG_")) {
                sb.setLength(0);
                sb.append("    ");
                sb.append(appIdToPackageMap.mapUid(e.callingUid));
                sb.append(',');
                sb.append(appIdToPackageMap.mapUid(e.workSourceUid));
                sb.append(',');
                sb.append(e.className);
                sb.append('#');
                sb.append(e.methodName);
                sb.append(',');
                sb.append(e.screenInteractive);
                sb.append(',');
                sb.append(e.cpuTimeMicros);
                sb.append(',');
                sb.append(e.maxCpuTimeMicros);
                sb.append(',');
                sb.append(e.latencyMicros);
                sb.append(',');
                sb.append(e.maxLatencyMicros);
                sb.append(',');
                if (this.mDetailedTracking) {
                    exportedCallStats2 = exportedCallStats;
                    j = e.exceptionCount;
                } else {
                    exportedCallStats2 = exportedCallStats;
                    j = 95;
                ...
                sb.append(j);
                sb.append(',');
                sb.append(this.mDetailedTracking ? e.maxRequestSizeBytes : 95L);
                sb.append(',');
                sb.append(this.mDetailedTracking ? e.maxReplySizeBytes : 95L);
                sb.append(',');
                sb.append(e.recordedCallCount);
                sb.append(',');
                sb.append(e.callCount);
                pw.println(sb);
        ...
        if (workSourceUid == -1) {
            ...
            verbose3 = verbose2;
            entries.sort(Comparator.comparingDouble(BinderCallsStats$$ExternalSyntheticLambda4.INSTANCE).reversed());
        ...
        List<UidEntry> summaryEntries = verbose3 ? entries : getHighestValues(entries, BinderCallsStats$$ExternalSyntheticLambda5.INSTANCE, 0.9d);
        Iterator<UidEntry> it = summaryEntries.iterator();
        while (true) {
            List<UidEntry> summaryEntries2 = summaryEntries;
            if (!it.hasNext()) {
                break;
            }
            UidEntry entry = it.next();
            String uidStr = appIdToPackageMap.mapUid(entry.workSourceUid);
            long totalRecordedCallsCount4 = totalRecordedCallsCount;
            long totalRecordedCallsCount5 = entry.cpuTimeMicros;
            pw.println(String.format("  %10d %3.0f%% %8d %8d %s", Long.valueOf(entry.cpuTimeMicros), Double.valueOf((totalRecordedCallsCount5 * 100.0d) / totalCpuTime), Long.valueOf(entry.recordedCallCount), Long.valueOf(entry.callCount), uidStr));
        ...
        long totalRecordedCallsCount6 = totalRecordedCallsCount;
        pw.println();
        if (workSourceUid == -1) {
            pw.println(String.format("  Summary: total_cpu_time=%d, calls_count=%d, avg_call_cpu_time=%.0f", Long.valueOf(totalCpuTime), Long.valueOf(totalCallsCount), Double.valueOf(totalCpuTime / totalRecordedCallsCount6)));
            pw.println();
        }
        ...
        exceptionEntries.sort(BinderCallsStats$$ExternalSyntheticLambda0.INSTANCE);
