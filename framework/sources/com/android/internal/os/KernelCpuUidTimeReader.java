    ...
    public static class KernelCpuUidUserSysTimeReader extends KernelCpuUidTimeReader<long[]> {
        ...
        */
        void readDeltaImpl(Callback<long[]> cb) {
            ...
            char c = 1;
            KernelCpuProcStringReader.ProcFileIterator iter = kernelCpuUidUserSysTimeReader.mReader.open(!kernelCpuUidUserSysTimeReader.mThrottle);
            if (iter == null) {
                if (iter != null) {
                    iter.close();
                    return;
            ...
            while (true) {
                try {
                    ...
                    if (buf == null) {
                        ...
                        jArr2[0] = currUsrTimeUs - lastTimes[0];
                        jArr2[c] = currSysTimeUs - lastTimes[c];
                        if (jArr2[0] >= 0 && jArr2[c] >= 0) {
                            if (jArr2[0] <= 0 && jArr2[c] <= 0) {
                        ...
                        Slog.e(kernelCpuUidUserSysTimeReader.mTag, "Negative user/sys time delta for UID=" + uid + "\nPrev times: u=" + lastTimes[0] + " s=" + lastTimes[1] + " Curr times: u=" + currUsrTimeUs + " s=" + currSysTimeUs);
                        lastTimes[0] = currUsrTimeUs;
                        lastTimes[1] = currSysTimeUs;
                        callback = cb;
                        c = 1;
