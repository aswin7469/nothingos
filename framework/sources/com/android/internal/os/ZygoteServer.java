...
public class ZygoteServer {
    ...
    private void fetchUsapPoolPolicyProps() {
        if (this.mUsapPoolSupported) {
            ...
            String usapPoolSizeMinPropString = Zygote.getConfigurationProperty(ZygoteConfig.USAP_POOL_SIZE_MIN, USAP_POOL_SIZE_MIN_DEFAULT);
            ...
            String usapPoolRefillDelayMsPropString = Zygote.getConfigurationProperty(ZygoteConfig.USAP_POOL_REFILL_DELAY_MS, USAP_POOL_REFILL_DELAY_MS_DEFAULT);
        ...
        while (true) {
            ...
            try {
                ...
                if (this.mUsapPoolRefillAction != UsapPoolRefillAction.NONE) {
                    boolean isPriorityRefill = true;
                    int[] sessionSocketRawFDs = socketFDs.subList(1, socketFDs.size()).stream().mapToInt(ZygoteServer$$ExternalSyntheticLambda0.INSTANCE).toArray();
                    if (this.mUsapPoolRefillAction != UsapPoolRefillAction.IMMEDIATE) {
                        isPriorityRefill = false;
                    }
