...
public final class CarrierAppUtils {
     ...
     */
    /* JADX WARN: Removed duplicated region for block: B:133:0x008d A[EXC_TOP_SPLITTER, SYNTHETIC] */
    ...
    */
    public static void disableCarrierAppsUntilPrivileged(String callingPackage, TelephonyManager telephonyManager, ContentResolver contentResolver, int userId, Set<String> systemCarrierAppsDisabledUntilUsed, Map<String, List<CarrierAssociatedAppEntry>> systemCarrierAssociatedAppsDisabledUntilUsed, Context context) {
        ...
        if (candidates != null && !candidates.isEmpty()) {
            ...
            int carrierAppsHandledSdk4 = Settings.Secure.getIntForUser(contentResolver2, str4, 0, userId2);
            ...
            try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            try {
                                        ...
                                        if (telephonyManager2.checkCarrierPrivilegesForPackageAnyPhone(packageName) == 1) {
                                            ...
                                            if (!hasPrivileges) {
                                                try {
                                                    ...
                                                    Log.i(TAG, "Update state (" + packageName + str5 + userId);
                                                    context.createContextAsUser(UserHandle.of(userId), 0).getPackageManager().setSystemAppState(packageName, 2);
                                                    context.createPackageContextAsUser(str3, 0, UserHandle.of(userId)).getPackageManager().setApplicationEnabledSetting(packageName, 1, 1);
                                                    if (associatedAppList != null) {
                                                        Iterator<AssociatedAppInfo> it2 = associatedAppList.iterator();
                                                        while (it2.hasNext()) {
                                                            AssociatedAppInfo associatedApp2 = it2.next();
                                                            Iterator<AssociatedAppInfo> it3 = it2;
                                                            String str6 = str4;
                                                            int associatedAppEnabledSetting = context.createContextAsUser(UserHandle.of(userId), 0).getPackageManager().getApplicationEnabledSetting(associatedApp2.appInfo.packageName);
                                                            boolean associatedAppInstalled = (associatedApp2.appInfo.flags & 8388608) != 0;
                                                            try {
                                                                if (associatedAppEnabledSetting != 0) {
                                                                    carrierAppsHandledSdk2 = carrierAppsHandledSdk4;
                                                                    if (associatedAppEnabledSetting != 4 && associatedAppInstalled) {
                                                                        str2 = str5;
                                                                        it2 = it3;
                                                                        str4 = str6;
                                                                ...
                                                                Log.i(TAG, "Update associated state (" + associatedApp2.appInfo.packageName + str5 + userId);
                                                                str2 = str5;
                                                                context.createContextAsUser(UserHandle.of(userId), 0).getPackageManager().setSystemAppState(associatedApp2.appInfo.packageName, 2);
                                                                context.createPackageContextAsUser(str3, 0, UserHandle.of(userId)).getPackageManager().setApplicationEnabledSetting(associatedApp2.appInfo.packageName, 1, 1);
                                                                it2 = it3;
                                                                str4 = str6;
                                                                carrierAppsHandledSdk4 = carrierAppsHandledSdk2;
                                                                str5 = str2;
                                                ...
                                                try {
                                                    if (!isUpdatedSystemApp(ai2) && enabledSetting == 0 && (ai2.flags & 8388608) != 0) {
                                                        ...
                                                        context.createContextAsUser(UserHandle.of(userId), 0).getPackageManager().setSystemAppState(packageName, 3);
                                                    }
                                                    if (associatedAppList != null) {
                                                        for (AssociatedAppInfo associatedApp3 : associatedAppList) {
                                                            if (hasRunEver) {
                                                                if (hasRunForSdk || associatedApp3.addedInSdk == -1) {
                                                                    ai = ai2;
                                                                    carrierAppsHandledSdk3 = carrierAppsHandledSdk;
                                                                } else {
                                                                    carrierAppsHandledSdk3 = carrierAppsHandledSdk;
                                                                    if (associatedApp3.addedInSdk > carrierAppsHandledSdk3) {
                                                                        ai = ai2;
                                                                        if (associatedApp3.addedInSdk <= Build.VERSION.SDK_INT) {
                                                                        }
                                                                    } else {
                                                                        ai = ai2;
                                                                    }
                                                                ...
                                                                allowDisable = false;
                                                                int associatedAppEnabledSetting2 = context.createContextAsUser(UserHandle.of(userId), 0).getPackageManager().getApplicationEnabledSetting(associatedApp3.appInfo.packageName);
                                                                boolean associatedAppInstalled2 = (associatedApp3.appInfo.flags & 8388608) == 0;
                                                                if (allowDisable || associatedAppEnabledSetting2 != 0 || !associatedAppInstalled2) {
                                                                    c = 3;
                                                                } else {
                                                                    Log.i(TAG, "Update associated state (" + associatedApp3.appInfo.packageName + "): DISABLED_UNTIL_USED for user " + userId);
                                                                    c = 3;
                                                                    context.createContextAsUser(UserHandle.of(userId), 0).getPackageManager().setSystemAppState(associatedApp3.appInfo.packageName, 3);
                                                                }
                                                            ...
                                                            allowDisable = true;
                                                            int associatedAppEnabledSetting22 = context.createContextAsUser(UserHandle.of(userId), 0).getPackageManager().getApplicationEnabledSetting(associatedApp3.appInfo.packageName);
                                                            if ((associatedApp3.appInfo.flags & 8388608) == 0) {
                                                            }
                                                            if (allowDisable) {
                                                            }
                                                            c = 3;
                                ...
                                int enabledSetting2 = context.createContextAsUser(UserHandle.of(userId), 0).getPackageManager().getApplicationEnabledSetting(packageName);
                ...
                try {
                    permissionManager4.grantDefaultPermissionsToEnabledCarrierApps(packageNames, UserHandle.of(userId), TelephonyUtils.DIRECT_EXECUTOR, CarrierAppUtils$$ExternalSyntheticLambda0.INSTANCE);
