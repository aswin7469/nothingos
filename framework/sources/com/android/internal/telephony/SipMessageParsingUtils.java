...
public class SipMessageParsingUtils {
    ...
    public static Set<String> getAcceptContactFeatureTags(String headerString) {
        ...
        for (Pair<String, String> header : headers) {
            ...
            if (splitParams.length >= 2) {
                char c = 1;
                Set<String> fts = (Set) Arrays.asList(splitParams).subList(1, splitParams.length).stream().map(SipMessageParsingUtils$$ExternalSyntheticLambda0.INSTANCE).filter(SipMessageParsingUtils$$ExternalSyntheticLambda4.INSTANCE).collect(Collectors.toSet());
                for (String ft : fts) {
    ...
    private static boolean verifySipRequest(final String[] request) {
        ...
        try {
            if (Uri.parse(request[1]).getScheme() != null) {
                z = true;
            }
            boolean verified = z;
            return verified & Arrays.stream(SIP_REQUEST_METHODS).anyMatch(new Predicate() { // from class: com.android.internal.telephony.SipMessageParsingUtils$$ExternalSyntheticLambda3
