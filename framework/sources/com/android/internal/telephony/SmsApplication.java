...
public final class SmsApplication {
    ...
    private static Collection<SmsApplicationData> getApplicationCollectionInternal(Context context, int userId) {
        ...
        List<ResolveInfo> respondServices = packageManager2.queryIntentServicesAsUser(new Intent(TelephonyManager.ACTION_RESPOND_VIA_MESSAGE, Uri.fromParts(SCHEME_SMSTO, "", null)), 786432, UserHandle.of(userId));
    ...
    private static void defaultSmsAppChanged(Context context) {
        ...
        assignExclusiveSmsPermissionsToSystemApp(context, packageManager, appOps, BLUETOOTH_PACKAGE_NAME, true);
    ...
    private static void replacePreferredActivity(PackageManager packageManager, ComponentName componentName, String scheme) {
        ...
        List<ComponentName> components = (List) resolveInfoList.stream().map(SmsApplication$$ExternalSyntheticLambda1.INSTANCE).collect(Collectors.toList());
