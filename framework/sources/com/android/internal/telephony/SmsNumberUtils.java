...
public class SmsNumberUtils {
    ...
    */
    ...
    public static String filterDestAddr(Context context, int subId, String destAddr) {
        ...
        String result = null;
        if (needToConvert(context, subId) && (networkType = getNetworkType(telephonyManager)) != -1 && !TextUtils.isEmpty(networkOperator) && (networkMcc2 = networkOperator.substring(0, 3)) != null && networkMcc2.trim().length() > 0) {
            result = formatNumber(context, destAddr, networkMcc2, networkType);
        }
        if (destAddr.startsWith("91") && getNetworkType(telephonyManager) != -1 && !TextUtils.isEmpty(networkOperator) && (networkMcc = networkOperator.substring(0, 3)) != null && networkMcc.trim().length() > 0 && networkMcc.equals("404")) {
            result = PLUS_SIGN + destAddr;
        }
        if (z) {
