...
public class LockPatternUtils {
    ...
    public boolean setLockCredential(LockscreenCredential newCredential, LockscreenCredential savedCredential, int userHandle) {
        ...
        try {
            ...
            if (newCredential.getType() != -1) {
                i = new String(newCredential.getCredential()).length();
            }
            int pwdLength = i;
            setLong(NOTHING_PASSWORD_LENGTH, pwdLength, userHandle);
            compatOldPasswordLength();
            return true;
        } catch (RemoteException e) {
    ...
    private void compatOldPasswordLength() {
        if (SystemProperties.getInt("persist.sys.password.length", -1) > -1) {
            SystemProperties.set("persist.sys.password.length", "-1");
        }
    }
}
