...
public class NotificationActionListLayout extends LinearLayout {
    ...
    private void rebuildMeasureOrder(int capacityText, int capacityOther) {
        ...
        for (int i = 0; i < childCount; i++) {
            View c = getChildAt(i);
            if ((c instanceof TextView) && ((TextView) c).mo3084getText().length() > 0) {
    ...
    public static final class TextViewInfo {
        ...
        TextViewInfo(TextView textView) {
            ...
            this.mTextLength = textView.mo3084getText().length();
            this.mTextView = textView;
        }

        boolean needsRebuild() {
            return (this.mTextView.mo3084getText().length() == this.mTextLength && NotificationActionListLayout.isPriority(this.mTextView) == this.mIsPriority) ? false : true;
        }
    }
}
