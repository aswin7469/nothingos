...
public class PeopleHelper {
    ...
    public void animateViewForceHidden(final CachingIconView view, final boolean forceHidden) {
        ...
        float f = 0.5f;
        float f2 = 1.0f;
        ViewPropertyAnimator scaleX = view.animate().scaleX(forceHidden ? 0.5f : 1.0f);
        if (!forceHidden) {
            f = 1.0f;
        }
        ...
        scaleY.alpha(f2).setInterpolator(forceHidden ? MessagingPropertyAnimator.ALPHA_OUT : MessagingPropertyAnimator.ALPHA_IN).setDuration(160L);
    ...
    public Icon createAvatarSymbol(CharSequence name, String symbol, int layoutColor) {
        ...
        if (symbol.isEmpty() || TextUtils.isDigitsOnly(symbol) || SPECIAL_CHAR_PATTERN.matcher(symbol).find()) {
            Icon avatarIcon = Icon.createWithResource(this.mContext, (int) R.drawable.messaging_user);
